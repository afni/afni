    ...
    if not (tests_data_dir / ".datalad").exists():
        try:
            ...
            if not (tests_data_dir / ".datalad").exists():
                ...
                datalad.install(
                    path=str(tests_data_dir),
                    source="https://github.com/afni/afni_ci_test_data.git",
                    recursive=True,
                    on_failure="stop",
                )
        finally:
...
def save_output_to_repo(config_obj):
    ...
    ### might replace the above with this, bc of datalad syntax change:
    result = datalad.save(
        message=update_msg,
        path=str(base_comparison_dir_path),
        on_failure="stop",
    )
