...
def phase_base(amp_type, phasee):
    ...
    # Time series time vector.  This goes up monotomically and linearly as t = ki
    #   where t is teh time, k is a constant and i the array index.
    ...
    # phasee["t"] increases linearly with index but with a different constan from
...
def phase_estimator(amp_phase, phase_info):
    ...
    if isinstance(phasee["phasee_list"], type([])):
        ...
        for phasee_column in phasee["phasee_list"]:
            return_phase_list.append(phase_base(amp_phase, phasee_column))
