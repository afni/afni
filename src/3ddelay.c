...
void indexTOxyz ( DELAY_options* ud, int ncall, 
                  int *xpos , int *ypos , int *zpos);

void xyzTOindex (struct DELAY_options* option_data, int *ncall, 
                 int xpos , int ypos , int zpos);     
...

{
   ...
   if (Read_part_file_delay (option_data->rvec, option_data->ideal_filename[0], 
                             option_data->NFirst,option_data->NLast) <= 0) {
...

{
   ...
   for (ixyz = 0;  ixyz < nxyz;  ixyz++)  {
      ...
      */
      ...
      option_data->errcode = hilbertdelay_V2 (vox_vect, 
                                              option_data->rvec,
                                              option_data->ln,
                                              option_data->Nseg,
                                              option_data->Pover,
      ...
      if (option_data->errcode == 0) { /* If there are no errors, proceed */
         ...
         hunwrap (delu, (float)(option_data->fs), option_data->T, 
                  slp, option_data->wrp, option_data->unt, 
                  option_data->rev, option_data->scl, &del );
         ...
         if ((actv == 1) && (option_data->out == YUP)) {      
            /* if voxel is truly activated, write results to file
               without modifying return value */
            
            indexTOxyz ( option_data , ixyz, &xpos , &ypos , &zpos);
            fprintf (option_data->outwrite,
                     "%d\t%d\t%d\t%d\t%f\t%f\t%f\t%f\t%f\n", 
                     ixyz , xpos , ypos , zpos ,  delu , del, 
                     xcor , xcorCoef , vts);
            if (option_data->outts == YUP)  {
