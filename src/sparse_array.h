...
/* function for creating a sparse array from the thresholded
   ...
   inputs:
    ...
    note:
        this function can use a _lot_ of memory if you the sparsity is too high, we implement
        a crude check to try and avoid killing the system, but it may not always work!*/
sparse_array_node* create_sparse_corr_array( MRI_vectim* xvectim, double sparsity, double threshold, double (*corfun)(long,float*,float*), long memory_allowance  );
#endif
