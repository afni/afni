...
void Qhelp(void)
{
  ...
  printf(
    ...
    "                with 3dAllineate.\n"
    " -nopadWARP   = If you do NOT use '-nopad', then then computed warp will be bigger\n"
    "                than the base volume.  This situation is normally not an issue,\n"
    "                but if you require the warp volume to match the base volume,\n"
    "                then use '-nopadWARP' to have the output WARP dataset truncated\n"
    "                (i.e., assuming that zero-padding took place).\n"
#ifdef USE_SAVER
...
STATUS("adjust for 3dAllineate matrix") ;
       ...
       IW3D_destroy(oiw->warp) ; oiw->warp = tarp ;
     }
     ...
     if( !nodset ){
       ...
       mri_free(oiw->im) ; oiw->im = iim ; DSET_delete(iset) ; DSET_delete(wset) ;
   ...
   /*----- if needed, invert the output warp now -----*/
   ...
   if( zeropad ){
     ...
     if( oiw->im->nx > DSET_NX(bset) || oiw->im->ny > DSET_NY(bset) || oiw->im->nz > DSET_NZ(bset) ){
       qim = mri_zeropad_3D( -pad_xm,-pad_xp , -pad_ym,-pad_yp , -pad_zm,-pad_zp , oiw->im ) ;
       mri_free(oiw->im) ; oiw->im = qim ;
     }
     /* crop the output warp (maybe) */
     if( !zeropad_warp ){
       if( Hverb ) INFO_message("un-zero-padding warp back to original grid") ;
       QQ = IW3D_zeropad( oiw->warp , -pad_xm,-pad_xp , -pad_ym,-pad_yp , -pad_zm,-pad_zp  ) ;
       IW3D_destroy(oiw->warp) ; oiw->warp = QQ ;
       if( owwi != NULL ){
         QQ = IW3D_zeropad( owwi , -pad_xm,-pad_xp , -pad_ym,-pad_yp , -pad_zm,-pad_zp  ) ;
         IW3D_destroy(owwi) ; owwi = QQ ;
       }
     }
     /* same stuff for the plusminus warp as well */
     if( qiw != NULL ){
       if( qiw->im->nx > DSET_NX(bset) || qiw->im->ny > DSET_NY(bset) || qiw->im->nz > DSET_NZ(bset) ){
         qim = mri_zeropad_3D( -pad_xm,-pad_xp , -pad_ym,-pad_yp , -pad_zm,-pad_zp , qiw->im ) ;
         mri_free(qiw->im) ; qiw->im = qim ;
       }
       if( !zeropad_warp ){
         QQ = IW3D_zeropad( qiw->warp , -pad_xm,-pad_xp , -pad_ym,-pad_yp , -pad_zm,-pad_zp  ) ;
   ...
   if( owwi != NULL ) IW3D_adopt_dataset( owwi      , adset ) ;
