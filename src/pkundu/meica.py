...
icaopts=OptionGroup(parser,"Extended ICA options")
...
icaopts.add_option('',"--sourceTEs",dest='sourceTEs',help="Source TEs for ICA (see tedana.py -h)",default='0')
icaopts.add_option('',"--daw",dest='daw',help="Dimensionality increase weight",default='10')
parser.add_option_group(icaopts)
runopts=OptionGroup(parser,"Run optipns")
runopts.add_option('',"--cpus",dest='cpus',help="Maximum number of CPUs/threads to use. Default 2.",default='2')
runopts.add_option('',"--label",dest='label',help="Extra label to tag this ME-ICA folder.",default='')
...
print "-- Multi-Echo Independent Components Analysis (ME-ICA) v2.0 --"
...
if options.anat!='' and commands.getstatusoutput('3dinfo %s' % (options.anat))[0]!=0:
	print "*+ Can't find/load dataset %s !" % (options.anat)
	notfound+=1
if notfound!=0:
...
def dep_check():
	...
	if commands.getstatusoutput("3dinfo")[0]!=0:
		print "*+ Can't find AFNI binaries. Make sure AFNI is on the path!"
		fails+=1
	if commands.getstatusoutput('3dinfo -tr %s%s%s%s' % (prefix,datasets[0],trailing,isf))[0]!=0:
		print "*+ This seems like an old version of AFNI. Please upgrade to latest version of AFNI."
		fails+=1
	if fails==0:
		print " + All dependencies seem OK. Continuing with preprocessing."
	else:
		print "*+ EXITING. Please see error messages."
		sys.exit()
if not options.skip_check: dep_check()
...
if options.anat!='':
	...
	addslabs = int(cmdif/maxvoxsz)+10
...
if options.anat!='':
    ...
    #sl.append("3dcalc -a Segsy/Posterior+orig[1] -expr 'step(a-0.5)*a' -prefix epigraywt%s" % (osf))
    ...
    elif oblique_mode: align_args = " -cmass -maxrot 30 -maxshf 30 "
    ...
    #sl.append("3dAllineate -weight_frac 1.0 -VERB -warp aff -weight eBmask_pve_0.nii.gz -lpc+ZZ -base eBbase.nii.gz -source_automask+2 -master SOURCE -source %s -prefix ./%s_al -1Dmatrix_save %s_al_mat %s" % (alnsmprage, anatprefix,anatprefix,align_args))
    sl.append("3dAllineate -weight_frac 1.0 -VERB -warp aff -weight epigraywt%s -lpc -base Segsy/AnatUB+orig -source_automask+2 -master SOURCE -source %s -prefix ./%s_al -1Dmatrix_save %s_al_mat %s" % (osf,alnsmprage, anatprefix,anatprefix,align_args))
...
for echo_ii in range(len(datasets)):
    ...
    if options.tpattern!='':
        ...
        tpat_opt = ''
    sl.append("3dTshift -heptic %s -prefix ./%s_ts+orig %s/%s%s" % (tpat_opt,dsin,startdir,indata,isf) )
    ...
    if echo_ii == 0: 
        ...
        sl.append("3dAutomask -peels 3 -overwrite -prefix eBvrmask%s _eBvrmask%s" % (osf,osf))
    ...
    if options.FWHM=='0mm': 
        sl.append("3dTcat -prefix ./%s_sm%s ./%s_vr%s[%i..$]" % (dsin,osf,dsin,osf,basebrik))
    else: 
    ...
    if not (options.test_proc or options.keep_int): sl.append("rm %s_ts+orig* %s_vr%s %s_sm%s" % (dsin,dsin,osf,dsin,osf))
