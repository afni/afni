...
/************************************************************************
  This file is an OpenMP (OMP) compatible version of the
    venerable mixed-radix FFTN function.
  This file is meant to be #include-d into another C file,
    and it does NOT require that USE_OMP be defined, nor does
    it require being compiled with OpenMP enabled.
  It uses some macros in file Aomp.h to re-define global static scalars
    as arrays, so that they can be thread-safe. If Aomp.h is not
    available to you, the macro definitions needed are given below.
  To be compatible with the AJJ/RWC function
      csfft_cox( int mode , int idim , complex *xc )
    instead do this:
      fftnf_OMP( idim, NULL, &(xc[0].r), &(xc[0].i), 2*mode, 0.0 )
    or use the csfft_OMP() macro defined below
  As a side note, the SUN_BROKEN_REALLOC code has been removed,
    since no one uses the Sun C compiler anymore, and for that
    matter, no one uses Sun computers anymore.
  -- RWCox == @AFNIman -- 10 Nov 2020
************************************************************************/

/*=====================================================================*/
#ifndef _SECOND_ROUND_INCLUDE_
#define _SECOND_ROUND_INCLUDE_

#define USING_FFTN

#ifdef USE_OMP
# include "Aomp.h"      /* part of the AFNI software package */

/**** Below are the Aomp.h macros used herein;
      'AO' is an abbreviation for 'AFNI OpenMP'.
      If you don't want AFNI code polluting your life, then throw out
      the #include just above and instead enable the definitions below. ****/
# if 0
...
#define FFT_NODOUBLE    /* use only the float version fftnf() */

#ifndef PROTO_FFNTF_OMP
#define PROTO_FFNTF_OMP
int fftnf_OMP( int ndim, const int dims[],
               float Re[], float Im[], int iSign, double scaling ) ;
#endif

/* macro to make fftnt look like csfft,
   for ease of porting code from AFNI-land */

#define csfft_OMP(mode,idim,xc)                                        \
  fftnf_OMP( (idim), NULL, &((xc)[0].r), &((xc)[0].i), 2*(mode), 0.0 )

/*---------------------------------------------------------------------*/
/* Return the next highest integer that is of the form
     2^a * 3^b * 5^c
   where a >= 1 (i.e., value is even; at least one factor of 2)
   and   b <= 1 (i.e., at most one factor of 3)
   and   c <= 1 (i.e., at most one factor of 5)
   These values are efficient for FFTs, while
   giving some extra flexibility over just powers of 2.
   Code is adapted from the csfft_nextup() function in csfft.c.
*//*-------------------------------------------------------------------*/

int fftn_nextup_one35( int idim )
{
   int ibase=2 ;
   while(1){  /* loop over ibase=powers of 2 */
      if(              idim <=    ibase   ) return    ibase   ;
      if( 4 < ibase && idim <=  5*ibase/4 ) return  5*ibase/4 ; /* c=1 */
      if( 2 < ibase && idim <=  3*ibase/2 ) return  3*ibase/2 ; /* b=1 */
      if( 8 < ibase && idim <= 15*ibase/8 ) return 15*ibase/8 ; /* b=c=1 */
      ibase *= 2 ; /* next power of 2 */
   }
   return -666 ; /*unreachable*/
}

/*---------------------------------------------------------------------*/
/* Return the smallest integer that is even, greater than or equal
   to idim, and has no prime factors except 2, 3, and 5 -- but any
   number of powers of 3 and 5 are OK, unlike the previous function.
*//*-------------------------------------------------------------------*/

int fftn_nextup_even( int idim )
{
   int ibase=idim , itemp ;
   if( ibase   <= 2 ) return 2;
   if( ibase%2 == 1 ) ibase++ ; /* make it even */
   while(1){
     itemp = ibase ;    /* start here and see what happens below */
     while( itemp%4 == 0 ){ itemp /= 4 ; }  /* divide out all 4s */
     if   ( itemp%2 == 0 ){ itemp /= 2 ; }  /* any remaining 2?  */
     while( itemp%3 == 0 ){ itemp /= 3 ; }  /* divide out all 3s */
     while( itemp%5 == 0 ){ itemp /= 5 ; }  /* divide out all 5s */
     if( itemp == 1 ) return ibase ; /* nothing was left == GOOD */
     ibase += 2 ;            /* otherwise, try next even integer */
   }
   return -666 ; /*unreachable*/
}

#endif /* _SECOND_ROUND_INCLUDE_ */
...
#ifdef USE_OMP
AO_DEFINE_SCALAR(size_t,SpaceAlloced) ;
AO_DEFINE_SCALAR(size_t,MaxPermAlloced) ;
#else
...
#ifdef USE_OMP
AO_DEFINE_ARRAY(void,Tmp0) ;
AO_DEFINE_ARRAY(void,Tmp1) ;
AO_DEFINE_ARRAY(void,Tmp2) ;
AO_DEFINE_ARRAY(void,Tmp3) ;
AO_DEFINE_ARRAY(int,Perm)  ;
#else
...
#define NFACTOR	11
#ifdef USE_OMP
AO_DEFINE_ARRAY(int,factor) ;   /* must be allocated later */
#else
static int factor [NFACTOR];    /* pre-allocated */
#endif
/*}}}*/
...
#pragma omp critical (MALLOC)
 {
   AO_VALUE(SpaceAlloced) = AO_VALUE(MaxPermAlloced) = 0;
   if (AO_VALUE(Tmp0)) { free (AO_VALUE(Tmp0)); AO_VALUE(Tmp0) = NULL; }
   if (AO_VALUE(Tmp1)) { free (AO_VALUE(Tmp1)); AO_VALUE(Tmp1) = NULL; }
   if (AO_VALUE(Tmp2)) { free (AO_VALUE(Tmp2)); AO_VALUE(Tmp2) = NULL; }
   if (AO_VALUE(Tmp3)) { free (AO_VALUE(Tmp3)); AO_VALUE(Tmp3) = NULL; }
   if (AO_VALUE(Perm)) { free (AO_VALUE(Perm)); AO_VALUE(Perm) = NULL; }
 }
#else
...
#ifdef USE_OMP
   int *factor ;
   if( AO_ARRAY_LEN(factor) < NFACTOR ){    /* this code will be used */
#pragma omp critical (MALLOC)               /* once per thread to make */
     AO_RESIZE_ARRAY(int,factor,NFACTOR) ;  /* the permanent factor[] array */
   }
   factor = AO_VALUE(factor) ;
#endif
...
/* defines for float */
# define REAL		float
# define FFTN		fftnf_OMP		/* trailing 'f' for float */
# define FFTNS		"fftnf_OMP"		/* name for error message */
...
#pragma omp critical (MALLOC)
 {
#ifdef USE_OMP
   if (AO_VALUE(SpaceAlloced) < maxFactors * sizeof (REAL))
     {
	     AO_VALUE(SpaceAlloced) = maxFactors * sizeof (REAL);
	     AO_VALUE(Tmp0) = realloc (AO_VALUE(Tmp0), 2*AO_VALUE(SpaceAlloced));
	     AO_VALUE(Tmp1) = realloc (AO_VALUE(Tmp1), 2*AO_VALUE(SpaceAlloced));
	     AO_VALUE(Tmp2) = realloc (AO_VALUE(Tmp2), 2*AO_VALUE(SpaceAlloced));
	     AO_VALUE(Tmp3) = realloc (AO_VALUE(Tmp3), 2*AO_VALUE(SpaceAlloced));
     }
#else
   if (SpaceAlloced < maxFactors * sizeof (REAL))
     {
	     SpaceAlloced = maxFactors * sizeof (REAL);
	     Tmp0 = realloc (Tmp0, 2*SpaceAlloced);
	     Tmp1 = realloc (Tmp1, 2*SpaceAlloced);
	     Tmp2 = realloc (Tmp2, 2*SpaceAlloced);
	     Tmp3 = realloc (Tmp3, 2*SpaceAlloced);
     }
#endif
   else
     {
...
#ifdef USE_OMP
	maxFactors = AO_VALUE(SpaceAlloced) / sizeof (REAL);
#else
	maxFactors = SpaceAlloced / sizeof (REAL);
#endif
     }
#ifdef USE_OMP
   if ( AO_VALUE(MaxPermAlloced) < maxPerm )
     {
      AO_VALUE(Perm) = realloc (AO_VALUE(Perm), 2*maxPerm * sizeof(int));
      AO_VALUE(MaxPermAlloced) = maxPerm;
     }
#else
   if ( MaxPermAlloced < maxPerm )
     {
      Perm = realloc (Perm, 2*maxPerm * sizeof(int));
	...
	/* allow full use of alloc'd space */
#ifdef USE_OMP
	maxPerm = AO_VALUE(MaxPermAlloced);
#else
...
#if USE_OMP
   if (!AO_VALUE(Tmp0) || !AO_VALUE(Tmp1) ||
       !AO_VALUE(Tmp2) || !AO_VALUE(Tmp3) || !AO_VALUE(Perm)) goto Memory_Error;
   /* assign pointers */
   Rtmp = (REAL *) AO_VALUE(Tmp0);
   Itmp = (REAL *) AO_VALUE(Tmp1);
   Cos  = (REAL *) AO_VALUE(Tmp2);
   Sin  = (REAL *) AO_VALUE(Tmp3);
#else
