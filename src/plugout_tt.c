...
****************************************************************
  The file "trusted_hosts.h" controls which systems are
  allowed to connect to AFNI.  Currently, this is limited
  to systems at MCW Biophysics and to localhost.
****************************************************************
  Compilation:
...
static char afni_verbose = 0 ;  /* print out debug info? */
...
int main( int argc , char * argv[] )
{
   ...
   if( argc == 2 && strncmp(argv[1],"-help",5) == 0 ){
      printf("Usage: plugout_tt [-host name] [-v]\n"
             ...
             "  -host name  Means to connect to AFNI running on the\n"
             "                remote computer 'name'.  The default is\n"
             "                to connect on the current host.\n"
             "  -v          Verbose mode: prints out lots of stuff.\n"
            ) ;
      exit(0) ;
   }
...
int afni_io(void)
{
   ...
   if( afni_mode == AFNI_OPEN_DATA_MODE ){
      ...
      /** decide name of data channel:
            use shared memory (shm:) on localhost,
            use TCP/IP (tcp:) on other computer systems;
        * Note that the TCP/IP port number (here 8001) can be
      ...
      if( strcmp(afni_host,"localhost") == 0 )
         strcpy( afni_iocname , "shm:test_plugout:1K+1K" ) ;
      else
         sprintf( afni_iocname , "tcp:%s:8001" , afni_host ) ;
      ...
      /** write the command to AFNI into the buffer:
            ...
            * TT_XYZ_OUT means 'send me T-T coordinates';
            * PONAME means 'use this string for messages';
            * IOCHAN means 'use this I/O channel from now on'. **/

      sprintf( afni_buf , "TT_XYZ_OUT\n"
                          "PONAME aManCalledHorse\n"
                          "IOCHAN %s" ,
               afni_iocname ) ;
