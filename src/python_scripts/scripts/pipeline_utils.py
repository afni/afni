                    ...
                    for retval_index in rv_indices:
                        retval = tuple(
                            [
                                ...
                                *retval[retval_index + 1 :],
                            ]
                        )  # the rest of the retval
...
def run_check_afni_cmd(cmd_str, ps, in_dict, message=""):
    ...
    if "chdir" in keys:
        ...
        raise ValueError(
            "chdir (the directory for the command execution) is "
    ...
    if "shell_obj" in keys:
        ...
        if shell_obj.eo != ps.oexec:
            raise ValueError(
                "shell_obj was defined in the calling function."
    ...
    run_criteria = (
        ps.rewrite or ps.dry_run() or need_stdout or not (all(files_status.values()))
    )
...
class ShellComFuture(ab.shell_com):
    ...
    def __init__(
        self, com, eo="", capture=1, default_text="Text from future", trim_length=2000
    ):
        ...
        super().__init__(
            com, eo=eo, capture=capture, save_hist=0, trim_length=trim_length
        )
...
class PipelineConfig:
    def __init__(self, label):
        ...
        self.bokeh_port = (
            8787
        )  # default port to show graphical Bokeh debugging info with Dasks
    ...
    def init_opts(self):
        ...
        self.valid_opts.add_opt("-dsets", -1, [], helpstr="Names of datasets")
        self.valid_opts.add_opt(
            "-init_base", 1, [], helpstr="Name of initial base dataset"
        )
        self.valid_opts.add_opt(
        ...
        self.valid_opts.add_opt(
            "-prep_only", 0, [], helpstr="Do preprocessing steps only without alignment"
        )
        self.valid_opts.add_opt(
            "-help", 0, [], helpstr="The main help describing this program with options"
        )
        self.valid_opts.add_opt(
            "-limited_help",
            0,
        ...
        self.valid_opts.add_opt(
            "-option_help", 0, [], helpstr="Help for all available options"
        )
        self.valid_opts.add_opt(
            "-version", 0, [], helpstr="Show version number and exit"
        )
        self.valid_opts.add_opt("-ver", 0, [], helpstr="Show version number and exit")
        self.valid_opts.add_opt(
            "-verb", 1, [], helpstr="Be verbose in messages and options"
        )
        self.valid_opts.add_opt(
            "-save_script", 1, [], helpstr="save executed script in given file"
        )
        ...
        self.valid_opts.add_opt("-no_strip", 0, [], helpstr="Do not remove skull")
        self.valid_opts.add_opt(
            "-no_unifize",
            0,
            [],
        ...
        self.valid_opts.add_opt(
            "-unifize_template", 0, [], helpstr="Unifize mean templates"
        )
        self.valid_opts.add_opt(
            "-anisosmooth", 0, [], helpstr="anisotropically smooth mean templates"
        )
        self.valid_opts.add_opt(
            "-findtypical_final",
            0,
        ...
        self.valid_opts.add_opt(
            "-final_space", 1, [], helpstr="Provide name of '-space ..' field"
        )
        ...
        self.valid_opts.add_opt(
            "-rigid_only", 0, [], helpstr="Only do rigid alignment step"
        )
        self.valid_opts.add_opt(
            "-affine_only", 0, [], helpstr="Only do affine alignment step"
        )
        self.valid_opts.add_opt(
            "-nl_only", 0, [], helpstr="Only do nonlinear alignment step"
        )
        self.valid_opts.add_opt(
            "-nl_level_only",
            1,
        ...
        self.valid_opts.add_opt("-overwrite", 0, [], helpstr="Overwrite existing files")
        self.valid_opts.add_opt(
            "-dask_mode",
            1,
            ["None"],
            ["None", "SLURM", "localcluster"],
        ...
        self.valid_opts.add_opt(
            "-cluster_queue", 1, [], helpstr="SLURM queue partition (norm,nimh,...)"
        )
        self.valid_opts.add_opt(
            "-cluster_constraint",
            1,
        ...
        self.valid_opts.add_opt(
            "-cluster_walltime", 1, [], helpstr="SLURM walltime limit (36:0:0,...)"
        )
        self.valid_opts.add_opt(
            "-cluster_memory", 1, [], helpstr="SLURM cluster node memory minimum (20g)"
        )
        self.valid_opts.add_opt(
            "-warpsets",
            -1,
        ...
        self.valid_opts.add_opt(
            "-fs_seg_sets", -1, [], helpstr="Names of FreeSurfer segmentation datasets"
        )
        self.valid_opts.add_opt(
            "-aniso_iters",
            1,
        ...
        self.valid_opts.add_opt("-viz_only", 0, [], helpstr="Visualize graph only")
    ...
    def exists_msg_python(self, dsetname=""):
        raise ValueError("** Dataset: %s already exists ** Not overwriting." % dsetname)
    ...
    def save_history(self, dset, exec_mode):
        ...
        cmdline = au.args_as_command(sys.argv, '3dNotes -h "', '" %s' % dset.input())
        com = ab.shell_com("%s\n" % cmdline, exec_mode)
        com.run()
    ...
    def self_help(self, help_level=0):
        ...
        if help_level:
            ...
            for opt in self.valid_opts.olist:
                ...
                if opt.deflist:
                    print("   %-20s   %s" % ("   default:", str.join(" ", opt.deflist)))
        return 1
    ...
    def process_input(self):
        ...
        opt = self.user_opts.find_opt("-do_freesurf_mpm")
        if opt != None:
            self.do_freesurf_mpm = 1
        ...
        if opt:
            ...
            for warpset_name in self.warpsets.parlist:
                ...
                if not check_dset.exist():
                    self.error_msg(
                        "Could not find warp dset\n %s " % check_dset.input()
                    )
                else:
                    self.info_msg("Found warp dset %s\n" % check_dset.input())
        ...
        if not self.basedset.exist():
            self.error_msg(
                "Could not find initial base dataset\n %s " % self.basedset.input()
            )
        else:
            self.info_msg("Found initial base dset %s\n" % self.basedset.input())
        ...
        if opt != None:
            ...
            if not self.resizebase.exist():
                self.error_msg(
                    "Could not find resize base dataset\n %s " % self.resizebase.input()
                )
            else:
                self.info_msg("Found resize base dset %s\n" % self.resizebase.input())
        ...
        if opt:
            ...
            for fs_seg_set_name in self.fs_seg_sets.parlist:
                ...
                if not check_dset.exist():
                    self.error_msg(
                        "Could not find FreeSurfer segmentation dset\n %s "
                        % check_dset.input()
                    )
                else:
                    self.info_msg(
    ...
    def __repr__(self):
        return "<%s %s label=%r>" % (self.__class__.__name__, hex(id(self)), self.label)
...
class TemplateConfig(PipelineConfig):
    def __init__(self, label):
        ...
        self.findtypical_level = (
            []
        )  # no typical subject intermediates by default in nonlinear
