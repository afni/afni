...
ps = pu.TemplateConfig('make_template_dask.py')
ps.init_opts()
ps.version()
...
# this function seems to be obsolete - replaced by cluster.scale instead
#        cluster.start_workers(n_workers) 
        ...
        while ((client.status == "running") and (len(client.scheduler_info()["workers"]) < min_workers)):
            ...
            print("Waiting for a sufficient number of workers. Currently have %d, waiting for %d" % (current_pool_size, min_workers))
            sleep(1.0)
