...
class SingleSubjectWindow(QtGui.QMainWindow):
   ...
   def cb_help_main(self):
      ...
      self.update_help_window(g_help_string, title='uber_subject.py: main help')
   ...
   def cb_show_ap_command(self):
      ...
      status, wstr, mesg = ap_command_from_svars(self.svars, verb=self.verb)
   ...
   def reset_svars(self, svars=None):
      ...
      if not self.svars.uber_dir:
         self.set_svar('uber_dir', USTUFF.get_uber_results_dir())
...
def ap_command_from_svars(svars, verb=1):
   ...
   # create command, save it (init directory tree?), show it
   apsubj = USTUFF.AP_Subject(svars, verb=verb)
