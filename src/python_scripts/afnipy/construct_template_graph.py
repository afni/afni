...
def copy_base(ps, dset=None):
    """
    make +tlrc view AFNI format dataset if NIFTI input
    """
    ...
    if dset.type == "NIFTI":
        ...
        out_dict = pu.run_check_afni_cmd(
            cmd_str, ps, {"dset_1": o}, "** ERROR: Could not copy using"
        )
        o = out_dict["dset_1"]
    else:
        o = dset
...
def align_centers(ps, dset=None, basedset=None, suffix="_ac"):
    ...
    out_dict = pu.run_check_afni_cmd(cmd_str, ps, {"dset_1": o})
...
def skullstrip(ps, dset=None, suffix="_ns"):
    ...
    out_dict = pu.run_check_afni_cmd(cmd_str, ps,
...
def rigid_align(dset, base, ps=None, suffix="_4rigid"):
    ...
    # pair is used for the dictionary passed to pu.run_check_afni_cmd
    input_name = dset.fn
    outname = o.fn
    base_in = base.ppve()
    # remove temp
...
def affine_align(dset, base, suffix="_aff", aff_type="affine", ps=None):
    ...
    {rigid_opt} -prefix {out_prefix} -pad_input 15 -OK_maxite -maxite 50\
    """
...
def aniso_smooth(ps, dset=None, suffix="_as", iters="1"):
    ...
    print("executing:\n %s" % cmd_str)
    out_dict = pu.run_check_afni_cmd(
        cmd_str, ps, {"dset_1": o, "chdir": chdir}, "** ERROR: Could not anisotropically smooth using"
    )
    o = out_dict["dset_1"]
...
def upsample_dset(ps, dset=None, suffix="_rs"):
    ...
    print("executing:\n %s" % cmd_str)
    out_dict = pu.run_check_afni_cmd(
        cmd_str, ps, {"dset_1": o, "chdir": chdir}, "** ERROR: Could not upsample using"
    )
    o = out_dict["dset_1"]
...
def resample_dset(ps, dset, base, suffix="_rs"):
    ...
    print("executing:\n %s" % cmd_str)
    out_dict = pu.run_check_afni_cmd(
        cmd_str, ps, {"dset_1": o, "chdir": chdir}, "** ERROR: Could not resample using"
    )
    o = out_dict["dset_1"]
...
def min_dim_dset(ps, dset=None):
    ...
    shell_obj = pu.ShellComFuture(cmd_str, eo=ps.oexec, default_text="1 0 -1")
    out_dict = pu.run_check_afni_cmd(
        cmd_str,
        ps,
        {"shell_obj": shell_obj},
...
def rescale_affx_brain(ps, dset, suffix="_rescld", preprefix=""):
    ...
    omask = pu.prepare_afni_output(ps, dset, suffix="_mask")
    ...
    o = pu.prepare_afni_output(ps, dset, suffix)
...
def get_mean_brain(dset_list, ps, dset_glob, suffix="_rigid", preprefix=""):
    ...
    """
    print("getting mean for suffix %s\n" % suffix)
...
def get_rigid_mean(ps, basedset, dsetlist, delayed):
    ...
    # make AFNI +tlrc format copy if NIFTI input
    nbase = delayed(copy_base)(ps,dset=basedset)

    for dset in dsetlist:
        # start off just aligning the centers of the datasets
        aname = delayed(align_centers)(ps, dset=dset, basedset=nbase)
        amname = delayed(skullstrip)(ps, dset=aname)
        dname = delayed(unifize)(ps, dset=amname)
        af_aligned = delayed(rigid_align)(dname, base=nbase, ps=ps, suffix=rigid_suffix)
...
def nl_align(ps, dset, base, iniwarpset, **kwargs):
    ...
    warpset = pu.prepare_afni_output(o, "WARP")
    input_name = dset.initname
    out_prefix = str(Path(o.rps) / o.out_prefix())
...
#    aff_matrix = "%s.Xaff12.1D" % rsz_brain.rbn
    ...
    # check if output dataset was created
    rsz_warp = pu.run_check_afni_cmd(
...
def compute_deformation_dist(ps, aa_brain, warp, suffix="_defdist"):
    ...
    # create output dataset structure
    o = pu.prepare_afni_output(ps, aa_brain, suffix)

    warp_name = warp.pv()
    out_prefix = o.out_prefix()
    # inverse warp to compute distance in affine space (before nonlinear warp)
    inv_warp = pu.prepare_afni_output(ps, warp, "_inv")
    ...
    inv_warp = pu.run_check_afni_cmd(
        cmd_str, ps, inv_warp, "Could not compute inverse warp using"
    )

    # fill holes in brain
    filled_brain = pu.prepare_afni_output(ps, aa_brain, "_filled")
    ...
    # check if output dataset was created
    filled_brain = pu.run_check_afni_cmd(
        cmd_str, ps, filled_brain, "Could not fill holes using"
    )

    zp_inv_warp = pu.prepare_afni_output(ps, warp, "_inv_zp")
    ...
    cmd_str = cmd_str.format(**locals())
    # check if inverse warp output dataset was created
    zp_inv_warp = pu.run_check_afni_cmd(
...
def compute_deformation_dist(ps, aa_brain, warp, suffix="_defdist"):
    ...
    """
    # create output dataset structure
    o = pu.prepare_afni_output(aa_brain, suffix)

    warp_name = warp.pv()
    out_prefix = o.out_prefix()
    # inverse warp to compute distance in affine space (before nonlinear warp)
    inv_warp = pu.prepare_afni_output(warp, "_inv")
    inv_warp_prefix = inv_warp.out_prefix()
    inv_warp_name = inv_warp.pv()
    ...
    # fill holes in brain
    filled_brain = pu.prepare_afni_output(aa_brain, "_filled")
    input_name = aa_brain.pv()
    filled_brain_name = filled_brain.pv()
    filled_brain_prefix = filled_brain.out_prefix()
    ...
    zp_inv_warp = pu.prepare_afni_output(warp, "_inv_zp")
    zp_inv_warp_prefix = zp_inv_warp.out_prefix()
    zp_inv_warp_name = zp_inv_warp.pv()
...
def warp_fs_seg(ps, fs_seg, aa_brain, warp, suffix="_warped"):
    ...
    fs_seg_out = pu.prepare_afni_output(fs_seg, suffix)
    """
    ...
    cmd_str = cmd_str.format(**locals())
    # check if output dataset was created
    fs_seg_out = pu.run_check_afni_cmd(
        cmd_str,
        ps,
        {"dset_1": fs_seg_out},
...
def get_indata(ps, dsetlist, outdir, delayed):
    ...
    #    indir.mkdir()
    for d in dsetlist:
        # output path name from subject name without possible dual exts
        # like .nii.gz
        # sub01.nii.gz -> sub01
        # anat2+tlrc.BRIK -> anat2

        dpathstr = str(outdir / Path(d.name).with_suffix("").with_suffix(""))
