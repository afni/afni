...
def get_all_3dinfo_dset_basic(fname) :
    '''Run 3dinfo on dset fname, for all possible options that are not in
list_3dinfo_ignore.  This runs relatively slowly, calling 3dinfo
approx. N times for N option flags.
...
which may be null.  See "get_all_3dinfo_dset_neatly()" for some
filtering/proc'ing of opts.
...
def get_all_3dinfo_dset_neatly(fname) :
    check, DD = get_all_3dinfo_dset_basic(fname)

    DD_new = {}
    if not(check) :
        check, DD_new = make_neater_3dinfo_dict(DD)
