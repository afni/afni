...
"""
    ...
    if not proc.user_opts.find_opt('-regress_basis'):
        ...
        print "    The basis function will be convolved with the stimulus\n"  \
          ...
          "              TENT(0,18,7) - deconvolution, with an expected\n"  \
          "                             reponse lasting 18 seconds after\n" \
          "                             each stimulus, with beta weights\n" \
          "                             locked to a 3 second TR grid over\n"\
          "                             6 intervals (so 7 tents)\n"
...
def get_stim_labels(proc, type):
    ...
    while True:  # break when done
        ...
        labels = []
        while nremain > 0 :
            print '++ (%d remain) please enter label(s): ' % nremain,
...
def get_stim_files(proc, type):
    ...
    while True:  # break when done
        ...
        files = []
        while nremain > 0 :
            print '++ (%d remain) please enter file name(s): ' % nremain,
