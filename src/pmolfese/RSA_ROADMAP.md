...
| **Time-resolved M/EEG–fMRI fusion (`-model_series`)** | ✅ Delivered: independent fixed-model and dependent same-participant time × space fusion maps, with BH FDR and one max-statistic null over the joint time × space family. | P1 | Done | Roadmap **F20** plus the dependent-series extension. `-model_series` retains the independent/fixed contract; `-model_series_subjects paired\|loo` selects each subject's own or held-out-group temporal RDM by exact label. Paired mode uses population subject-sign inference; overlapping LOO templates use synchronized condition-label inference for the fixed observed participant sample. Supports classic ordinary/runwise neural inputs, atlas/searchlight mapping, long-form labeled tables, and deterministic AFNI timepoint bricks. |
| **Native temporal RDM movies (`1dTrdm`)** | ✅ Delivered producer, temporal-inference, cross-temporal-estimation, feature-neighborhood, and dependent-fusion gates: labeled observation×feature×time inputs produce subject RDM movies, corrected fixed-model inference, representational-recurrence surfaces, cross-time crossnobis, explicit overlapping feature searches, and independent, paired, or leave-one-subject-out `3dRSA` bridges. | P1/P2 | Done (five release gates) | Correlation, cosine, Euclidean, and balanced crossnobis; mean/concatenated windows; population sign-flip or fixed-condition null; joint time or time×neighborhood BH/max-FWE; separate symmetric Pearson/Spearman RDM-dynamics and ordered-partition cross-time distance products; exact-label subject×time manifests; explicit axes/counts/provenance; row/graph/seed/thread/layout identity; live fixed and dependent 3dRSA round trips. |
...
| **Time-resolved M/EEG–fMRI fusion** | `-model_series` reads an ordered fixed/independent `TIME_LABEL MATRIX_FILE` list. For overlapping participants, `-model_series_subjects paired\|loo` reads a complete exact-label `SUBJ TIME_LABEL MATRIX_FILE` grid and selects either each subject's own EEG/MEG RDM or a group template that excludes that subject. Paired mode uses population subject-sign inference. Because LOO templates overlap, LOO uses synchronized condition-label inference for the fixed observed participant sample and rejects independent-subject sign flips/bootstrap. The long-form table preserves verbatim time labels; AFNI bricks use deterministic `t####` labels; BH FDR and max-statistic FWE cover the complete time × space family. Classic ordinary and runwise/crossnobis RSA work in atlas and searchlight analyses. |
| **Temporal RDM companion** | `1dTrdm` reads labeled observation×feature×time matrices, aligns subject/condition/partition axes, and writes subject-level temporal RDMs for correlation, cosine, Euclidean, or balanced crossnobis estimators. A labeled fixed model activates Pearson/Spearman inference: population-subject sign flips or synchronized fixed-condition relabeling supply raw p, BH q, and complete time or time×neighborhood max-FWE. Separately named descriptive products provide symmetric Pearson/Spearman RDM recurrence and ordered-independent-partition cross-time crossnobis while retaining the dyad axis; canonical outputs store only the unique time triangle and crossnobis diagonals exactly equal primary RDMs. A strict `Neighborhood Feature` graph supports overlapping sensor/source/generic-feature searches without assuming column adjacency. Explicit windows and sidecars preserve time, feature, graph, condition, counts, subject fits, inference, and provenance. `-model_series_out independent\|paired\|loo` writes the appropriate fixed, same-subject, or held-out-group bridge into `3dRSA`. |
