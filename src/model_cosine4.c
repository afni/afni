...
void conv_set_ref( int num , float * ref )
{
   if( num > 0 && ref != NULL ){ /*** if have inputs, make space & copy in ***/
      ...
      if( refnz == 0 )
         ERREX("model_cosine4: All zero reference timeseries!") ;
     ...
     if( cp == NULL )
        ERREX("model_cosine4: need ref file as AFNI_CONVMODEL_REF") ;

     flim = mri_read_1D(cp) ;      /* 16 Nov 1999: replaces mri_read_ascii */
     if( flim == NULL ){
        char buf[256] ;
        sprintf(buf,"model_cosine4: Can't read timeseries file %s",cp) ;
        ERREX(buf) ;
     }
...
MODEL_interface * initialize_model ()
{
  ...
  if( AFNI_yesenv("AFNI_MODEL_HELP_COSINE4") ||
...
static int model_help(void)
{
   printf(
"----------------------------------------------------------------------\n"
"cosine4    - sum of 4 half-cosine curves\n"
"\n"
