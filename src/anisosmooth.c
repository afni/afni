...
int main( int argc , char * argv[] )
{
   ...
   if( argc < 2 || strcmp(argv[1],"-help") == 0 ){
      printf("Usage: anisosmooth [options] dataset\n"
   ...
   mainENTRY("anisosmooth main"); machdep(); AFNI_logger("anisosmooth",argc,argv);
  ...
  for(i=0;i<iters;i++){
     ...
     if((i==iters-1)&&(save_tempdsets_flag)) {
       tross_Make_History ("anisosmooth", argc, argv, structtensor);
  ...
  tross_Make_History ("anisosmooth", argc, argv, udset);
...
{
  MRI_IMAGE *im;
  /*  char default_name[64] = "anisosmooth image";*/
  NI_element *nel;
