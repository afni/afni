...
static char *readme_env[] = {
   ...
   "\n" ,
   "A potential penalty for using mmap is when reading is not linear, such as\n" ,
   "when reading over the time dimension.  There have been reported filesystem\n" ,
   "bottlenecks resulting from too much disk access, leading to processing times\n" ,
   "being 50-300 times as long as without mmap, akin to thrashing when one runs\n" ,
   "out of RAM.  This is highly dependent on FS caching and other configuration\n" ,
   "choices.\n" ,
   "\n" ,
   ...
   "-----------------------------\n" ,
   "If this YES/NO variable is set to YES, then 2D image files\n" ,
   "(*.png and *.jpg) will be read as datasets when the interactive\n" ,
   "AFNI program starts.\n" ,
   "Image files can be opened when passed directly on the command line.\n" ,
   "Image files can also be opened using the Axial image viewer, aiv.\n" ,
   "\n" ,
   "----------------------------\n" ,
   ...
   "\n" ,
   "---------------------------\n" ,
   "Variable: AFNI_CROSSHAIRGAP\n" ,
   "---------------------------\n" ,
   "This numeric variable lets you set the gap between the image viewer\n" ,
   "crosshairs.  The default value is 5 voxels. Allowable range is from -1\n" ,
   "(closed) to 19 voxels.\n" ,
   "\n" ,
   "--------------------------\n" ,
   "Variable: AFNI_OVCROSSHAIR\n" ,
   "--------------------------\n" ,
   "This variable controls the color of the crosshair in the image windows\n" ,
   ...
   "the debugging level in whereami_afni routines. Currently used values range\n" ,
   "from 0 to 3.\n" ,
   "\n" ,
   "------------------------------\n" ,
   ...
   "-------------------------------\n" ,
   "Turns off warnings about various whereami_afni features - like queries that\n" ,
   ...
   "-------------------------------\n" ,
   "\n" ,
   "By default, whereami_afni searches a radius of 7.5 mm. Set a radius up to\n" ,
   "9.5 mm.\n" ,
   "\n" ,
   "-------------------------------\n" ,
   ...
   "coordinate input to whereami_afni, the whereami functionality in the GUI \n" ,
   "and for TLRC view datasets without a template space explicitly set in the \n" ,
   "dataset header.\n" ,
   "\n" ,
   "--------------------------------\n" ,
   ...
   "short name available. The whereami GUI functionality and whereami_afni\n" ,
   "command line program can show either or both labels with this variable\n" ,
   "set to \"longname\", \"both\" or \"name\", where \"name\" is only the short,\n" ,
   "standard version. The overlay panel and image viewers can show this\n" ,
   "label too. The names are separated by a vertical pipe symbol if both\n" ,
   "are in the atlas header.  The default is to show both names if they\n" ,
   "are available.\n" ,
   "\n" ,
   "-------------------------\n" ,
