static char * readme_env[] = {
   ...
   "for plugins, executable files, atlases, templates and atlas definition files. See the atlas environment variable section below for more details.  If there is more than one appropriate directory, they can be separated by colons, as in\n" ,
   "\n" ,
   ...
   "dynamically loaded shared objects. Take care to remove any slow or non-existent directories from the PATH because these problems will cause searches to fail or become excessively slow.\n" ,
   "\n" ,
   "------------------------\n" ,
   ...
   "-----------------------------------\n" ,
   "This integer sets the debugging level in whereami routines. \n" ,
   "Currently used values range from 0 to 3.\n" ,
   "\n" ,
   "\n" ,
   "=============================================\n" ,
   "| Robert W Cox, PhD                         |\n" ,
   ...
   "template spaces. The variables may also affect how other AFNI programs use atlases as input datasets.\n" ,
   "\n" ,
   "-------------------------\n" ,
   "Variable: AFNI_ATLAS_LIST \n" ,
   "-------------------------\n" ,
   "This list contains the names of the atlases that should be queried when no specific atlas has been requested. For example, the afni GUI and whereami, by default, do not load all the atlases specified in the AFNI_atlas_spaces.niml file. If this variable is\n" ,
   " not set, the TT_Daemon atlas and the cytoarchitectonic Eickhoff-Zilles in MNI_ANAT space are loaded. If the variable is set to a list like \"TT_Daemon,DD_Desai_PM\", then only these two atlases are loaded. The list of atlas names may be separated by commas\n" ,
   " or semicolons. A special case of \"ALL\" may be set, and all the available atlases will be loaded.\n" ,
   "\n" ,
   "----------------------------------\n" ,
   ...
   "\n" ,
   "----------------------------\n" ,
   "Variable: AFNI_DEFAULT_ATLAS (editable)\n" ,
   "----------------------------\n" ,
   "This variable sets which atlas to use in the AFNI GUI for \"Show Atlas Colors\", \"Go to Atlas location\", \"Draw Dataset\" and \"Renderer\" functions.\n" ,
   "\n" ,
   "-------------------------\n" ,
   "Variable: AFNI_PLUGINPATH\n" ,
   "-------------------------\n" ,
   "Described above, this variable is used for several purposes. For whereami, this variable sets the directory to load atlases and NIML files if not in the current directory. If this variable does not exist or the referred file does not exist, then the atlas\n" ,
   " is searched in the user's current PATH setting. If this is set, atlases will be found more quickly than searching all the directories of the entire PATH. Searches of the PATH variable for whereami usage stop at the first occurrence of the searched file, \n" ,
   "so placing the AFNI directory earlier will dramatically increase the speed that the file is found.\n" ,
   "\n" ,
   "----------------------------------\n" ,
   "Variable: AFNI_WHEREAMI_DEC_PLACES (editable)\n" ,
   "----------------------------------\n" ,
   "Sets precision for whereami output.  Higher field data and animal atlases require higher precision. The default value used for focus point among template spaces is still 0 decimal places (closest mm), but animal data requires three decimal places. Value m\n" ,
   "ay range from 0 to 10.\n" ,
   "\n" ,
   "-------------------------\n" ,
   "Variable: AFNI_WAMI_DEBUG\n" ,
   "-------------------------\n" ,
   ...
   "-------------------------------\n" ,
   "Turns off warnings about various whereami features - like queries that have reached their limit of returned results. By default, warnings are displayed only the first time a particular message is encountered.\n" ,
   "\n" ,
   "--------------------------------\n" ,
   ...
   "\n" ,
   "-------------------------------\n" ,
   "Variable: AFNI_WHEREAMI_MAX_RAD (editable)\n" ,
   "-------------------------------\n" ,
   ...
   "-------------------------------------------\n" ,
   "These variables allow the addition of more atlas definitions to the global list of atlases, templates, spaces and transformations. The variable should be set to the name of a NIML file with the same format of the AFNI_atlas_spaces.niml file. These can be \n" ,
   "customized by site (supplemental) or by subject (local) and follow the same search order as the AFNI_atlas_spaces.niml file. In order to be included in default searches, additional atlases or template spaces would also need to be added to AFNI_ATLAS_SPACE\n" ,
   "_LIST and the AFNI_TEMPLATE_SPACE_LIST unless those are set to \"ALL\".\n" ,
   "\n" ,
   "------------------------------\n" ,
