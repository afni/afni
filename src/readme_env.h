...
static char *readme_env[] = {
   ...
   "\n" ,
   "------------------------------\n" ,
   "Variable: AFNI_SOUND_NOTE_TYPE\n" ,
   "------------------------------\n" ,
   "This variable can be set to control the type of notes played by\n" ,
   "sox when using the 'p' or 'P' key in an AFNI graph viewer window.\n" ,
   "The values allowed here (from the 'sox' program) are\n" ,
   "  sine square triangle sawtooth trapezium pluck\n" ,
   "You can abbreviate the name to the first 3 characters, if you\n" ,
   "really want to save typing. The name must be in lower case letters;\n" ,
   "upper case letters (e.g., SINE) won't work.\n" ,
   "The default is 'pluck', which sounds sort of like a cross between\n" ,
   "a piano note and a guitar note. The other note types are more\n" ,
   "continuous sounding. You might have to adjust the sound gain to\n" ,
   "get the volume right for you when changing the note type.\n" ,
   "\n" ,
   "-------------------------\n" ,
   "Variable: AFNI_SOUND_GAIN\n" ,
   "-------------------------\n" ,
   "This numeric variable controls the sound volume. The default value\n" ,
   "is '-27'. Smaller (more negative) is quieter. The largest value\n" ,
   "allowed here is -1, which will be loud and your neighbors\n" ,
   "will not be happy.\n" ,
   "\n" ,
   "-----------------------------\n" ,
   ...
   "for above-threshold pixels down to the 'Floor' value (default is 0) --\n" ,
   "however, pixels with the threshold variable exactly equal to zero will\n" ,
   "not get the Floor value of opacity, but will get alpha=0. Note that\n" ,
   "the alpha (opacity) level at the pixel-wise level is scaled by the\n" ,
   "global opacity '1-9' control on the right side of the image viewer,\n" ,
   ...
   "this via variables AFNI_resam_func and AFNI_resam_thr).  Also when\n" ,
   "Alpha is turned on, the above-threshold pixel regions are outlined in\n" ,
   "black, unless you set the AFNI_EDGIZE_OVERLAY environment variable to\n" ,
   "NO -- this variable can be set interactively via the 'EditEnv'\n" ,
   "pseudo-plugin.  You can alter the color used for the outline by\n" ,
   "setting the AFNI_EDGIZE_COLOR variable; for example, afni\n" ,
   "-DAFNI_EDGIZE_COLOR=hotpink -DAFNI_FUNC_ALPHA=Linear\n" ,
   "\n" ,
   "The outline is done over the pixels immediately OUTSIDE the\n" ,
   "above-threshold regions, in the overlay image as interpolated to the\n" ,
   "underlay resolution.  If the underlay is on a coarse matrix (e.g.,\n" ,
   "native EPI), these outlines will look terrible -- you can alter the\n" ,
   "underlay display grid dimension in 'Define Datamode' using the control\n" ,
   "'Warp ULay on Demand'.\n" ,
   "\n" ,
   ...
   "\n" ,
   " * If Alpha is off, then edges shown comprise the outermost\n" ,
   "   above-threshold voxels. The edge color is just the color that these\n" ,
   "   voxels normally get from the overlay sub-brick values. In this\n" ,
   "   mode, the 'blobs' are hollowed out, leaving only the edges.\n" ,
   "\n" ,
   " * If the Alpha is on, the edges shown comprise the voxels just\n" ,
   "   OUTSIDE the outermost above-threshold voxels -- so that all the\n" ,
   "   above-threshold voxels are visible. Also, the edge color is solid\n" ,
   "   black (or can be set via variable AFNI_EDGIZE_COLOR). The 'blobs'\n" ,
   "   are NOT hollowed out.\n" ,
   "\n" ,
   "---------------------------\n" ,
   "Variable: AFNI_EDGIZE_COLOR (editable)\n" ,
   "---------------------------\n" ,
   "Defines the color used for the edge outline of above threshold\n" ,
   "regions, when Alpha is turned on.  See AFNI_FUNC_ALPHA for more\n" ,
   "information.  This variable has no effect if Alpha is turned off.\n" ,
   ...
   "If this variable is set to 'BIG' or a positive number, then a larger\n" ,
   "font size is used.\n" ,
   "If this variable is set to 'SMALL' or a negative number, then a\n" ,
   "smaller font size is used.\n" ,
   "\n" ,
   "-------------------------\n" ,
