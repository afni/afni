...
"\n"
     ...
     if( 1 ){   /* this used to be only for "-HELP" */
       ...
       printf("\n"
        ...
        "\n"
        " 1. Coarse Phase [Goal: explore the vastness of 6D space for starting points]\n"
        "  a. The first step uses only the first 6 parameters (shifts + rotations),\n"
        "     and tests a large number of potential starting points -- selected\n"
        "     from a 6D grid in parameter space and also from random points\n"
        "     in parameter space. This step is fairly slow. The best 45 parameter\n"
        "     sets (in the sense of the cost functional) are kept for the next step.\n"
        "     These optimizations (and those in 1b) use the blurring radius\n"
        "     given by option '-twoblur', which defaults to 7.77 mm, and uses\n"
        ...
        "     -- this does not happen often.) Output parameter sets of the 3rd pass\n"
        ...
        "     parameter set is added here to ensure against the possibility that the\n"
