...
"    if you know what you are doing (or just like to play).\n"
"\n"
"=====----------------------------------------------------------------------\n"
   ...
   while( iarg < argc && argv[iarg][0] == '-' ){
     ...
     if( strcmp(argv[iarg],"-nwarp") == 0 ){     /* 03 Apr 2008 = SECRET */
       ...
       if( iarg < argc && isalpha(argv[iarg][0]) ){
         ...
         } else if( strncasecmp(argv[iarg],"cub",3) == 0 ){       /* 13 Nov 2010 */
...
#define CTHRESH 0.02f
     ...
     if( nwarp_pass ){
       ...
       if( nwarp_type == WARP_BILINEAR ){  /*------ special case ------------*/
         ...
         if( verb > 0 ) INFO_message("Start Cubic warping") ;
...
#define PARAM_SETUP(pp,ff,vv)                                                           \
 do{ if( ff ){ stup.wfunc_param[pp].fixed = 1 ; stup.wfunc_param[pp].val_fixed = vv ; } \
