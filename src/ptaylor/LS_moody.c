...
/*
  ...
  (MIT Room E25-505A, Cambridge, MA 02139 USA).  For updates to this
  software, please visit PhysioNet (http://www.physionet.org/).
  ______________________________________________________________________________

  The default input to this program is a text file containing a
  sampled time series, presented as two columns of numbers (the sample
  times and the sample values).  The intervals between consecutive
  samples need not be uniform (in fact, this is the most significant
  advantage of the Lomb periodogram over other methods of power
  spectral density estimation).  This program writes the Lomb
  periodogram (the power spectral density estimate derived from the
  input time series) on the standard output.

  The original version of this program was based on the algorithm
  described in Press, W.H, and Rybicki, G.B., Astrophysical
  J. 338:277-280 (1989).  It has been rewritten using the versions of
  this algorithm presented in Numerical Recipes in C (Press,
  Teukolsky, Vetterling, and Flannery; Cambridge U. Press, 1992).

  This version agrees with 'fft' output (amplitude spectrum up to the
  Nyquist frequency with total power equal to the variance); thanks to
  Joe Mietus.
*/
             ...
             int m)
{
   ...
   if (m > 10)
      moody_error("factorial table too small");
   ix = (int)x;
...
void LS_calc_workspace_size( unsigned long n,
                             ...
                             unsigned long *ndim)
{
   unsigned long nfreq, nfreqt;
   
   *nout = 0.5*ofac*hifac*n;
   
   nfreqt = ofac*hifac*n*MACC; // use criterion in fasper.
   nfreq = 64;
   while (nfreq < nfreqt) 
      nfreq <<= 1;
   *ndim = nfreq << 1;
            ...
            float *prob) 
{
   ...
   float ave, ck, ckk, cterm, cwt, den, df, effm, expy, 
      fac, fndim, hc2wt,
   ...
   //ndim = nfreq << 1;
   //if (ndim > nwk)
   //   moody_error("workspaces too small\n");
   ...
   for (k = 3, j = 1; j <= (nout); j++, k += 2) {
      ...
      cwt = sqrt(0.5+hc2wt);
      swt = moody_SIGN(sqrt(0.5-hc2wt), hs2wt);
      den = 0.5*n + hc2wt*wk2[k] + hs2wt*wk2[k+1];
      cterm = moody_SQR(cwt*wk1[k] + swt*wk1[k+1])/den;
      sterm = moody_SQR(cwt*wk1[k+1] - swt*wk1[k])/(n - den);
      wk1[j] = j*df;
