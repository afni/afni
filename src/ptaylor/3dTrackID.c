	...
	while( iarg < argc && argv[iarg][0] == '-' ){
      ...
      if ( strcmp(argv[iarg],"-gdset_toy") == 0 ) {
         ...
         gset = SUMA_FloatVec_to_GDSET(mv, 2, 9, "full", labs, 
                                       NULL, NULL, NULL);
         /* Now add the XYZ of each graph point */
         ...
         /* Create some bundles representing edge 1-2=5 or 2-1=7*/
         ...
         tt = Free_Tracts(tt, 1);
         /* add it to network */
         net = AppAddBundleToNetwork(net, &tb, 5, NULL);
         /* make another one for edge 0-1*/
         tt = (TAYLOR_TRACT *)calloc(1, sizeof(TAYLOR_TRACT));
         ...
         tt = Free_Tracts(tt, 1);
         /* add it to network */
         net = AppAddBundleToNetwork(net, &tb, 1, NULL);
