...
"     -tract_out_mode MODE: Type of output format.\n"
"                           Choose from:\n"
...
"               e.g., AND]\n"
"               Use ALL to keep all tracks meeting minimum length criterion\n"
"               For now you still have to specify a mask even if you are using\n"
"               ALL, complain to Paul Taylor about this.\n"
"\n"
  ...
  for( k=0 ; k<Dim[2] ; k++ ) 
   ...
   if(coorded[i][j][k][3] >= MinFA) { 
     for( ii=0 ; ii<SeedPerV[0] ; ii++ ) 
      ...
      if( KEEPIT == 1 ) {
        tb = AppCreateBundle(tb, 1, tt, NULL); tt = Free_Tracts(tt, 1);
