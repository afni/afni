...
void usage_LombScargle(int detail) 
{
  printf(
"\n"
"  Make a (normalized) periodogram of time series with non-constant sampling.\n"
"\n"
...
"\n"
"  This particular implementation is due to G. Moody, part of an open source\n"
"  software toolbox (with GPL license; see website on C code pages) called \n"
"  PhysioToolkit: \n"
...
"                  {-mask MASK} {-in_upsamp N1} {-in_mult_nyq N2}\n"
"\n"
"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *\n"
"\n"
"  + RUNNING:\n"
...
int main(int argc, char *argv[]) {
   ...
   unsigned long  Npts_wrk = 0;   // output array size-- to be calc'ed
   unsigned long Npts_out, jmax;  // holders for output
  ...
  if( MASK ) 
     if ( Dim[0] != DSET_NX(MASK) || Dim[1] != DSET_NY(MASK) ||
          Dim[2] != DSET_NZ(MASK) ) {
        ERROR_message("Mask and inset don't appear to have same dimensions.\n");
        exit(1);
     }
  ...
  if( in_censor ) {
     ...
     // find out number of non-censored points, make float array of
     // times
     Npts_cen = 0;
     for( i=0; i<Dim[3] ; i++) {
        if(censor_sh[i]) {
     ...
     //ERROR_exit("Error: no censor file '-in_censor1D' input!  See help.");
     WARNING_message("NB: no censor file input-- "
                     "continuing with no censoring done!");
     
     Npts_cen = Dim[3];
     censor_flt = (float *)calloc(Npts_cen, sizeof(float));
     censor_sh = (short *)calloc(Npts_cen, sizeof(short));
     if( (censor_sh == NULL) || (censor_flt == NULL) ) {
        fprintf(stderr, "\n\n MemAlloc failure.\n\n");
        exit(233);
     }
  ...
  // calculate output size-- needs to be at least bigger than
  // hifac*ofac*Npts/2.
  //Npts_wrk = my_hifac * my_ofac * Npts_cen * 22;
  //if ( Npts_wrk % 2 )
  //   Npts_wrk++;
  //Npts_wrk/= 2;
  ...
  LS_calc_workspace_size( Npts_cen, 
                          my_ofac, 
                          my_hifac, 
                          &Npts_out,
                          &Npts_wrk);
  ...
  wk1 = (float *)calloc(Npts_wrk, sizeof(float));
  wk2 = (float *)calloc(Npts_wrk, sizeof(float));
