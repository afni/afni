...
"   voxel in the brain:\n"
"      + for each data set, the voxel is used as a seed to generate a \n"
"        correlation map within a user-defined mask (e.g., whole brain).\n"
"      + the correlation maps are (Pearson) correlated with each other, \n"
"        generating a single correlation coefficient \n"
"      + the correlation map coefficient is stored at the same voxel\n"
"        location in the output data set\n"
"   and the process is repeated.  Thus, the output is a whole brain map\n"
"   of correlation coefficients for corresponding voxels from the two data\n"
      ...
      for( j=0 ; j<Dim[1] ; j++ ) 
         for( i=0 ; i<Dim[0] ; i++ ) {
            if( mskd[i][j][k] ) {
               ...
               //INFO_message("%7d : corr=%f", nprog, scorrAB[ctr]);
