...
void
VPWarpA111N (in_image, in_width, in_height, in_bytes_per_scan,
				...
				/* [   1  ]                 [  1  ] */
{
	...
	while (scans_to_next_vertex > 0) {
	    ...
	    if (lft_zero_cnt > 0) {
		bzero(out_ptr, lft_zero_cnt * ((1) + (1)));
		out_ptr += lft_zero_cnt * ((1) + (1));
	    }
	...
	((out_ptr)[1]) = opc_acc_int;;
	    ...
	    if (rgt_zero_cnt > 0) {
		bzero(out_ptr, rgt_zero_cnt * ((1) + (1)));
		out_ptr += rgt_zero_cnt * ((1) + (1));
	    }

	    /* go on to next scan */
	    scans_to_next_vertex--;
	    out_scan_y++;
	    out_ptr += out_bytes_per_scan - out_width * ((1) + (1));
