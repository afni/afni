...
SUMA_Boolean SUMA_ScaleToMap_Interactive (   SUMA_OVERLAYS *Sover )
{
   ...
   if (Opt->bind >= 0) {
      ...
      fact = (Opt->BrightMap[1] - Opt->BrightMap[0]) / (maxB - minB) + Opt->BrightMap[0];
      for (i=0; i<Sover->dset_link->nel->vec_filled; ++i) {
         if (!SV->isMasked[i]) {
   ...
   for (i=0; i<Sover->dset_link->nel->vec_filled; ++i) {
      if (!SV->isMasked[i]) {
         cnt3 = 3*cnt; i3 = 3*i;
         Sover->NodeDef[cnt] = Sover->NodeDef[i];
