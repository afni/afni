...
//    for (i=0; i<25; ++i)
//        fprintf(stderr, "overlay->ColVec[i] = %f, %f, %f\n", i, overlay->ColVec[3*i], overlay->ColVec[3*i+1], overlay->ColVec[3*i+2]);
...
//                    fprintf(stderr, "glOldGlColar[%d] = %f, %f, %f\n", i, glcolar[4*i], glcolar[4*i+1], glcolar[4*i+2]);
          ...
          for (i=0; i < N_Node; ++i) {
             ...
             if (SO->BoxOutline && outlinevector[i]){
                   ...
                   avg_Back = (glcolar_Back[i4_0] + glcolar_Back[i4_1] +
                               glcolar_Back[i4_2])/3;
//                   avg_Back = (baseOverlay->ColVec[3*1] + baseOverlay->ColVec[3*1+1] +
//                               baseOverlay->ColVec[3*1+2])/6;
...
//                    (avg_Back * complement); ++i4;
             ...
             // if (0) {
                ...
                continue;
                   ...
                   fprintf(stderr, "11-Back: opacity = %f\n", opacity);
                   i4 = 4 * i;
                   // DEBUG
                   glcolar[i4] = (0.0f*opacity) +
                    (SUMA_GRAY_NODE_COLOR * complement); ++i4;
                   glcolar[i4] = (1.0f*opacity) +
                    (SUMA_GRAY_NODE_COLOR * complement); ++i4;
                   glcolar[i4] = (0.0f*opacity) +
                    (SUMA_GRAY_NODE_COLOR * complement); ++i4;
                   /*
...
SUMA_Boolean SUMA_MixColors (SUMA_SurfaceViewer *sv)
{
   ...
   for (i=0; i<sv->N_ColList; ++i){
       ...
       fprintf(stderr, "(SUMA_SurfaceObject *)(SUMAg_DOv[%d].OP)->idcode_str = %s\n", i+1, ((SUMA_SurfaceObject *)(SUMAg_DOv[i+1].OP))->idcode_str);
   }
#endif
