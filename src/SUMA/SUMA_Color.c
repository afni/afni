                                  ...
                                  int colind, char *task, int perc)
{
   ...
   if (!Sover->dset_link) {
      ...
      if (Sover->dset_link->dnel &&(Sover->dset_link->dnel &&
...
{
   static char FuncName[]={"SUMA_MakeThresholdOutlines"};
   float *B=NULL;
   int i, icmap, i3, cnt, cnt3, loc[2], *nd=NULL;
   float *nv = NULL, am = 0.0;
   double Range[2];
   ...
   B = NULL;
   /* Thresholding ? */
   /* Set element to masked if T value within \pm 
   the threshold AND either vairable thresholding false or threshold
   outlining true.  The condition is only possible when
   Opt->ThreshRange[0] > 0*/
   for (i=0; i<SDSET_VECFILLED(Sover->dset_link); ++i) {
       if (Sover->T[i] < Opt->ThreshRange[0] &&
           Sover->T[i] > -Opt->ThreshRange[0]) {
                                 ...
                                 SUMA_SurfaceViewer *SV, GLfloat *glcolar)
{
   ...
   SUMA_Boolean *isColored, *isColored_Fore = NULL, *isColored_Back = NULL;
   GLfloat *glcolar_Fore = NULL, *glcolar_Fore_tmp, *glcolar_Back = NULL;
   ...
   if ( N_Overlays > 0 && (!Overlays || (int)Overlays == 0x1) ) {
   ...
   if (NshowOverlays && NshowOverlays_Back) {
      ...
      /* Alpha mode: any check of AlphaOp should also check UseThr
   ...
   if (NshowOverlays && !NshowOverlays_Back) {
         ...
         && currentOverlay->OptScl->UseThr)
      {
         for (i=0; i < N_Node; ++i) {
            ...
            if (isColored_Fore[i] ) {
               if (opacity == 1.0f ) { /* a separate condition, just for speed */
                  glcolar[i4] = glcolar_Fore[i4]; ++i4;
                  glcolar[i4] = glcolar_Fore[i4]; ++i4;
                  glcolar[i4] = glcolar_Fore[i4]; ++i4;
                  isColored[i] = YUP;
