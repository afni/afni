...
char * g_sm_desc[] = {
    ...
    "associated node volume between surfs",
    "associated node volume between surfs via Gauss' theorem",
    "node number",
...
ENTRY("compute_node_vols_G");
    ...
    for ( node = 0; node < p->S.nnodes; node++ )
    {
        ...
        sop->NodeDim = 3;
        sop->FaceSetDim = 3;
        sop->NodeList = (float *)SUMA_calloc(sop->NodeDim*sop->N_Node,
                                             sizeof(float));
