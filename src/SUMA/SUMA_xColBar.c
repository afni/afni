    ...
    SUMA_OVERLAYS *over2, Bool thresholdChanged)
{
   ...
   if (over2){ // If there are at least three overlay levels (as teh thrid has
    ...
    // SO = (SUMA_SurfaceObject *)ado;
   AlphaOpacityFalloff = over2->AlphaOpacityFalloff = XmToggleButtonGetState (SO->SurfCont->AlphaOpacityFalloff_tb);
...
void SUMA_cb_BoxOutlineThresh_tb_toggled(Widget w, XtPointer data,
                                   XtPointer client_data)
{
    // NB: w = SO->SurfCont->BoxOutlineThresh_tb
...
void SUMA_CreateCmapWidgets(Widget parent, SUMA_ALL_DO *ado)
{
   ...
   SUMA_X_SurfCont *SurfCont=NULL;
   SUMA_Boolean LocalHead = NOPE;
   SUMA_OVERLAYS *curColPlane=SUMA_ADO_CurColPlane(ado);
   ...
   { /* the threshold bar */
      rct = XtVaCreateWidget ("rowcolumn",
        ...
        // Place "A" and "B" checkboxes just above sliding bar
        {
            ...
            // SurfCont->BoxOutlineThresh_tb = XtVaCreateManagedWidget("_", 
            xmToggleButtonWidgetClass, ABCheckBoxContainer,
            NULL);
