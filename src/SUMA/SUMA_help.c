...
char * SUMA_help_message_Info(void)
{
   ...
   SS = SUMA_StringAppend (SS, "\t  c: load a node color file.\n\n");
   ...
   SS = SUMA_StringAppend (SS, "\t  p: Viewer rendering mode (Fill, Line, Points), switch.\n\n");
   ...
   SS = SUMA_StringAppend (SS, "\t  shft+Button 3-Press: ROI drawing (when in DrawROI mode)\n");
   SS = SUMA_StringAppend (SS, "\n");
...
void SUMA_Version (FILE *Out)
{
	...
	fprintf (Out, "\n\tSUMA version 1.9, Mon Mar 31 15:40:04 EST 2003\n");
	return;
}
