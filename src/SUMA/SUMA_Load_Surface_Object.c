   ...
   switch (SO_FT) {
     ...
     case SUMA_BRAIN_VOYAGER:
      ...
      case SUMA_FREE_SURFER_PATCH:
         ...
         if (SO->FileFormat == SUMA_ASCII) {    // PDL: NB: This is where the ascii file,
                                      //  containing the surface specifications, is read
...
{  /* start SUMA_Load_Spec_Surf */
   ...
   if (!brk && SUMA_iswordin(Spec->SurfaceType[i], "Ply") == 1) {
      ...
      SO = SUMA_Load_Surface_Object_eng ( (void *)Spec->SurfaceFile[i], SUMA_PLY,
