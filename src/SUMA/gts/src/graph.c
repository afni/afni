...
static void graph_read (GtsObject ** object, GtsFile * f)
{
  if (f->type != GTS_STRING) {
    gts_file_error (f, "expecting a string (GtsGraphClass)");
    return;
  }
 ...
 * @fp: a #GtsFile.
 *
 * Adds to @g the data read from @fp. The format of the file pointed
 * to by @fp is as described in gts_graph_write().
 *
 * Returns: 0 if successful or the line number at which the parsing
 * stopped in case of error (in which case the @error field of @fp is set).
 */
guint gts_graph_read (GtsGraph * g, GtsFile * fp)
{
  ...
  gts_file_next_token (fp);
  if (fp->type != '\n')
    if (GTS_OBJECT (g)->klass->read)
      (* GTS_OBJECT (g)->klass->read) ((GtsObject **) &g, fp);
  gts_file_first_token_after (fp, '\n');
  if (nn <= 0)
    return 0;
