               ...
               if (Delete && iplane >=0) {
                  ...
                  */
                  SUMAg_CF->ClipPlaneType[SUMAg_CF->N_ClipPlanes] =
                                    (SUMA_CLIP_PLANE_TYPES)EngineData->i;
                  snprintf(SUMAg_CF->ClipPlanesLabels[SUMAg_CF->N_ClipPlanes], 8,
                           "%.*s", 7, EngineData->s);                  
                  ...
                  snprintf(SUMAg_CF->ClipPlanesLabels[iplane],
                                          8*sizeof(char), "%.*s", 7, EngineData->s);
                  SUMAg_CF->ClipPlanes[4*iplane  ] =
                                          (GLdouble)EngineData->fv15[0];
                  SUMAg_CF->ClipPlanes[4*iplane+1] =
                                          (GLdouble)EngineData->fv15[1];
            ...
            if (NI_get_attribute(EngineData->ngr, "I_range")) {
               ...
               if (!stmp) {
                  ...
                  if (nn < 1 || nn > 2) {
                     ...
                     SymIrange = (SurfCont->curColPlane->OptScl->IntRange[1] == 
                        -SurfCont->curColPlane->OptScl->IntRange[0]);
                     SurfCont->curColPlane->SymIrange = SymIrange;
            ...
            if (NI_get_attribute(EngineData->ngr,"N_Key")) {
               ...
               for (ii=0; ii<itmp; ++ii) {
                  ...
                  if (stmp && (nc = strlen(stmp))) {
                     ...
                     if (strstr(stmp, "Shift")) stmp[strlen(stmp)-1] =
                        toupper(stmp[strlen(stmp)-1]);
                     for (rep=0; rep<N_rep; ++rep) {
                        ...
                        switch (k) {
                           ...
                           case XK_space:
                              ...
                              break;
                           case XK_slash:
                              if (!SUMA_slash_Key(sv, stmp, "drivesuma")) {
                                 SUMA_S_Err("Failed in Key function.");
                              }
                              break;
                           case XK_period:
                           ...
                           case XK_comma:
                              ...
                              break;
                           case XK_plus:
                              if (!SUMA_plus_Key(sv, stmp, "drivesuma")) {
                                 SUMA_S_Err("Failed in Key function.");
                              }
                              break;
                           case XK_minus:
                              if (!SUMA_minus_Key(sv, stmp, "drivesuma")) {
                                 SUMA_S_Err("Failed in Key function.");
                              }
                              break;
                           case XK_equal:
                              if (!SUMA_equal_Key(sv, stmp, "drivesuma")) {
                                 SUMA_S_Err("Failed in Key function.");
                              }
                              break;
                           case XK_F1:
