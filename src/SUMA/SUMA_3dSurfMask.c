...
void usage_3dSurfMask (SUMA_GENERIC_ARGV_PARSE *ps)
{
   ...
   "  Voxels in the first volume (named PREFIX.m) are set to the following\n"
   "  values:\n"
...
{/* Main */    
   ...
   { float * isin_float = NULL;
      ...
      if (!dset) {
         ...
         char **lblv = (char **)SUMA_calloc(N_labels, sizeof(char*));
   ...
   dsetd = SUMA_VoxelToSurfDistances(SO, dset, NULL, isin);
