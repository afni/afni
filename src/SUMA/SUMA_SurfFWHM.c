...
static char uSFWHM_Example1[]={
   ...
   "\n"
   "     suma -niml &\n"
   "     set Niso = `CreateIcosahedron -rad 100 -ld 80 -nums_quiet`; set Niso = $Niso[1]\n"
   "     CreateIcosahedron -tosphere   -rad 100 -ld 80 -prefix sphere_iso_$Niso\n"
   "     DriveSuma  -com show_surf -label sphere_iso_$Niso \\\n"
   "                -i_fs sphere_iso_${Niso}_surf.asc\n"
   "\n"
   "     echo Create some noise on the sphere.\n"
   "     1deval -num $Niso -del 1 -expr 'gran(0,1)*10000' > ${Niso}_rand.1D.dset\n"
   "     DriveSuma  -com surf_cont -label sphere_iso_$Niso -load_dset ${Niso}_rand.1D.dset\\\n"
   "                -switch_dset ${Niso}_rand.1D.dset -T_sb -1\n"
   "\n"
   "     echo What is the global FWHM of the noise? -a sanity check-\n"
   "     set randFWHM = `SurfFWHM -i_fs sphere_iso_${Niso}_surf.asc \\\n"
   "                              -input ${Niso}_rand.1D.dset` ; echo $randFWHM \n"
   "\n"
   "     echo Now smooth the noise\n"
   "     set opref_rand = ${Niso}_rand_sm10 && rm -f ${opref_rand}.1D.dset \n"
   "     SurfSmooth -spec sphere_iso_$Niso.spec -surf_A sphere_iso_$Niso -met HEAT  \\\n"
   "                -input ${Niso}_rand.1D.dset -fwhm 10 -Niter 50 -output ${opref_rand}.1D.dset\n" 
   "     DriveSuma  -com surf_cont -label sphere_iso_$Niso \\\n"
   "                -load_dset ${opref_rand}.1D.dset -switch_dset ${opref_rand}.1D.dset -T_sb -1\n"
   "\n"
   ...
   "     1dplot -ylabel 'number of nodes' -x ${fwhmpref}_histog.1D'[0]' -xlabel 'Local FWHM'\\\n"
   "            -plabel \"(Mean,Global) =($mFWHM, $gFWHM)\" ${fwhmpref}_histog.1D'[1]' & \n"
   "\n"
   "     echo Notice that the FWHM estimates are different from what we put in SurfSmooth.\n"
   "     echo The problem lies in the approximations for the FWHM in the HEAT method. Better\n"
   "     echo use SurfSmoothToFWHM instead for better control on smoothing levels.\n"
   "\n"
...
void usage_SurfFWHM (SUMA_GENERIC_ARGV_PARSE *ps)
{
      ...
      printf ( "\n"
         ...
         /*    "                -detrend disables -dmed, and includes -unif.\n"
               ...
               " -vox_size D = Specify the nominal voxel size in mm. This is used\n"
               "               To reduce artifacts of oversampling caused by nearest-\n"
               "               neighbor interpolation of low-resolution voxels onto\n"
