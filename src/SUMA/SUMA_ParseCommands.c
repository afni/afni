...
SUMA_SO_File_Type SUMA_SurfaceTypeCode (char *cd)
{
   ...
   if (  !strcmp(cd, "Stl") || !strcmp(cd, "STL") || 
         !strcmp(cd, "stl")) { 
      SUMA_RETURN( SUMA_STL); 
   }
   ...
   if (  !strcmp(cd, "OBJ") || !strcmp(cd, "obj") ) { 
      SUMA_RETURN( SUMA_OBJ_MESH); 
   }
...
SUMA_SO_File_Type SUMA_guess_surftype_argv(char *str)
{
   ...
   if (SUMA_iswordin_ci(str, "dx")  == 1 ) 
      SUMA_RETURN( SUMA_OPENDX_MESH );
   if (SUMA_iswordin_ci(str, "obj")  == 1 ) 
      SUMA_RETURN( SUMA_OBJ_MESH );
   if (SUMA_iswordin_ci(str, "pre")  == 1 ) 
      SUMA_RETURN( SUMA_PREDEFINED );
   if (SUMA_iswordin_ci(str, "ply")  == 1 ) 
      SUMA_RETURN( SUMA_PLY );
   if (SUMA_iswordin_ci(str, "stl")  == 1 ) 
      SUMA_RETURN( SUMA_STL );
                                    ...
                                    char **pdspec, char **pdsv, char **pdsname)
{
   ...
   else if (  SUMA_isExtension(Name, ".obj")) form = SUMA_OBJ_MESH; 
   else if (  SUMA_isExtension(Name, ".stl")) form = SUMA_STL; 
	...
	while (kar < argc) { /* loop accross command ine options */
      ...
      if (!brk && ps->accept_i) {
         ...
         do {
            ...
            if (!brk && (  (strcmp(tmp_i, "-i_stl") == 0) || 
                           (strcmp(tmp_i, "-i_Stl") == 0) 
                           || (strcmp(tmp_i, "-i_STL") == 0))) {
               ps->arg_checked[kar]=1;
               ps->arg_checked[skar]=1;
               if (skar >= argc)  {
               ...
               ps->i_FT[ps->i_N_surfnames] = SUMA_STL;
      ...
      if (!brk && ps->accept_ipar) {
        ...
        if (!SUMA_isExtension(argv[kar], ".topo")) { 
         ...
         if (!brk && (  (strcmp(argv[kar], "-ipar_stl") == 0) || 
                        (strcmp(argv[kar], "-ipar_Stl") == 0) || 
                        (strcmp(argv[kar], "-ipar_STL") == 0))) {
            ps->arg_checked[kar]=1;
            kar ++; ps->arg_checked[kar]=1;
            if (kar >= argc)  {
            ...
            ps->ipar_surfnames[ps->ipar_N_surfnames] = 
                                       SUMA_copy_string(argv[kar]);
            ps->ipar_FT[ps->ipar_N_surfnames] = SUMA_STL;
         ...
         if (!brk && (  (strcmp(argv[kar], "-ipar_OBJ") == 0) || 
                        (strcmp(argv[kar], "-ipar_obj") == 0) || 
                        (strcmp(argv[kar], "-ipar_Obj") == 0))) {
            ps->arg_checked[kar]=1;
            kar ++; ps->arg_checked[kar]=1;
            if (kar >= argc)  {
            ...
            ps->ipar_surfnames[ps->ipar_N_surfnames] = 
                                       SUMA_copy_string(argv[kar]);
            ps->ipar_FT[ps->ipar_N_surfnames] = SUMA_OBJ_MESH;
            ps->ipar_FF[ps->ipar_N_surfnames] = SUMA_ASCII;
            ++ps->ipar_N_surfnames;
            brk = YUP;
         }
      ...
      if (!brk && ps->accept_t) {
         ...
         if (!brk && (strcmp(argv[kar], "-tsn") == 0)) {
            ...
            if (  ps->t_FT[ps->t_N_surfnames] == SUMA_SUREFIT ||
                  ...
                  ps->t_FT[ps->t_N_surfnames] == SUMA_OBJ_MESH ||
            ...
            else if (ps->t_FT[ps->t_N_surfnames] == SUMA_STL)
                  ps->t_FF[ps->t_N_surfnames] = SUMA_FF_NOT_SPECIFIED;
