...
{
   int nxy=256 , ntr=200 , ndiv=16 , nbb=3 , iarg ;
   float TR=2.5f , blur=3.0f , base=30.0f , bsig=3.0f ;
   float amx=0.8f , bmx=0.5f , amn=0.0f , bmn=-0.5f ;
   float taumx=3.0f , sigmx=5.0f ;
   MRI_IMAGE *rim=NULL ;
   char *prefix="Simm" ;

   THD_3dim_dataset *dset ;
   THD_ivec3 ixyz ; THD_fvec3 dxyz ;
   int dvx,dvy,xx,yy,tt , kk ;
   ...
   if( argc == 1 || strcmp(argv[1],"-help") == 0 ){
     printf(
      ...
      "-blr xx = FWHM of blurring to be applied to baseline [default=3   ]\n"
...
#define OOP(ostr,ovar)                                                         \
 if( strcmp(argv[iarg],(ostr)) == 0 ){                                         \
   ...
   INFO_message("build baseline model") ;
   ...
   for( yy=0 ; yy < nxy ; yy++ ){
    for( xx=0 ; xx < nxy ; xx++ ){
      ...
      THD_insert_series( xx+yy*nxy , dset , ntr , MRI_float , tsar , 1 ) ;
   }}
   ...
   if( blur > 0.0f ){
     ...
     for( tt=0 ; tt < ntr ; tt++ )
       EDIT_blur_volume( nxy,nxy,1 , 1.0f,1.0f,1.0f ,
   ...
   if( taumx > 0.0f && rim != NULL ){
     ...
     for( xx=0 ; xx < nxy ; xx++ ){
       ...
       for( yy=0 ; yy < nxy ; yy++ ){
         ...
         if( beta != 0.0f ){
           THD_extract_array( xx+yy*nxy , dset , 1 , tsar ) ;
   ...
   INFO_message("adding ARMA(1,1) model") ;
   kk = (nxy+nxy-2)/(2*ndiv) ; a0 = amn ; a1 = amx/kk ;
   kk = (nxy-1)    /(2*ndiv) ; b0 = 0.5f*(bmn+bmx) ; b1 = (bmx-bmn)/(2.0f*kk) ;
   kk = 1 + (nxy-1)/ndiv ; dsig = sigmx/kk ;
   for( yy=0 ; yy < nxy ; yy++ ){
     kk  = yy/ndiv ; val = (1+kk)*dsig ;
     for( xx=0 ; xx < nxy ; xx++ ){
