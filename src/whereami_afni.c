...
/**Original code by Mike Angstadt *******************************************
  ...
  usage: whereami_afni x y z [output format]
...
"               more than one atlas. Default is all available atlases.\n");
   if (detail > 1) {
      ...
      printf("              Use whereami_afni -help to see all available\n"
             "              atlases.\n");
   }
...
"     right-clicking in an image window.\n\n"
      ...
      printf("    Use whereami_afni -help to see more information on:\n"
             "      Convenient Color maps For Atlas Datasets:\n"
             "         and \n"
             "      How To See Atlas regions overlaid in the AFNI GUI:\n"
      );
...
"    whereami_afni -atlas aparc.a2009s+aseg_rank -show_atlas_code | grep -i insula\n"
...
int main(int argc, char **argv)
{
   ...
   mainENTRY("whereami_afni main"); machdep();
   AFNI_logger("whereami_afni",argc,argv);
         ...
         if (!(aar = ROI_String_Decode(shar, atlas_alist))) {
            ...
            if (nbest && write_mask) {
               ...
               if (!(maskset = Atlas_Region_Mask(aar, codes, n_codes,
                  ...
                  tross_Make_History( "whereami_afni" , argc, argv , maskset ) ;
                  if (mskpref) {
      ...
      if (!OldMethod) {
         ...
         if (string) fprintf(stdout,"%s\n", string);
            ...
            if(!get_wami_web_found())
               fprintf(stdout,"whereami_afni NULL string out.\n");
         }
         if (string) {free(string); string = NULL;}            
      }
   } /* ixyz */   
