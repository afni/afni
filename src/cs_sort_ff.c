...
/********************************************************************************/
/* insertion_sort : sort an array of float + float                              */

static void isort_floatfloat( int n , float * ar , float * iar )
{
   register int  j , p ;  /* array indices */
   register float temp ;  /* a[j] holding place */
   register float  itemp ;
   register float * a = ar ;
   register float * ia = iar ;
...
static void qsrec_floatfloat( int n , float * ar , float * iar , int cutoff )
{
   register int i , j ;           /* scanning indices */
   register float temp , pivot ;  /* holding places */
   register float ipivot ;
   register float * a = ar ;
   register float * ia = iar ;
   int itemp ;
   ...
   while( mst > 0 ){
      ...
      if( a[left] > a[i]     ){ QS_SWAPF(a[left] ,a[i]    ); QS_SWAPF(ia[left] ,ia[i]    ); }
      if( a[left] > a[right] ){ QS_SWAPF(a[left] ,a[right]); QS_SWAPF(ia[left] ,ia[right]); }
      if( a[i] > a[right]    ){ QS_SWAPF(a[right],a[i]    ); QS_SWAPF(ia[right],ia[i]    ); }
...
{
   qsrec_floatfloat( n , a , ia , QS_CUTOFF ) ;
   isort_floatfloat( n , a , ia ) ;
   return ;
}
