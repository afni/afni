...
float UC_unusuality( int ndim , float * ref , int nvec , float ** vec )
{
   ...
   /* compute dot products, convert to Fisher z */
   ...
   /* estimate mean and standard deviation into zmid and zsig (robustly) */

   qsort_float( nvec , zval ) ;
   if( nvec%2 == 1 )              /* median */
      zmid = zval[nvec/2] ;
   else
      zmid = 0.5 * ( zval[nvec/2] + zval[nvec/2-1] ) ;

   for( kk=0 ; kk < nvec ; kk++ ) aval[kk] = fabs(zval[kk]-zmid) ;
   qsort_float( nvec , aval ) ;
   if( nvec%2 == 1 )              /* MAD = median absolute deviation */
      zmed = aval[nvec/2] ;
   else
      zmed = 0.5 * ( aval[nvec/2] + aval[nvec/2-1] ) ;
   ...
   for( kk=0 ; kk < nvec ; kk++ ){
      if( zval[kk] > zplus ){
         val = phizero( zval[kk] , zmid , zsig ) ;
         if( val > 0.0 ) psum += ww( pstar/val ) ;
      } else if( zval[kk] < zminus ){
         val = phizero( zval[kk] , zmid , zsig ) ;
         if( val > 0.0 ) msum += ww( pstar/val ) ;
      }
   }
