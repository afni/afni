...
{
   int nopt=1,method=METHOD_PAIRWISE,summary=INSYNC_SUMMARY_MEDIAN,show_table=0 ;
   int corr_metric=SIM_PEARSON,missing_policy=MISSING_ERROR ;
   int quiet=0,progress_mode=PROGRESS_AUTO,memory_override=0 ;
   int memory_limit_given=0 ; double memory_limit_gib=0.0 ;
   int nboot=0,nperm=0,do_exact=0,tail=PERM_TAIL_TWO ;
   int temporal_mode=TNULL_NONE,nnull=0,min_shift=1,min_shift_given=0 ;
   int temporal_tail=PERM_TAIL_ONE,temporal_tail_given=0 ;
   int ncondperm=0,condition_exact=0,condition_tail=PERM_TAIL_TWO ;
   int condition_column_given=0,have_contrast=0,contrast_c0=-1,contrast_c1=-1 ;
   long seed=1234567L ; float boot_alpha=0.05f ;
   char *prefix="InSync",*maskname=NULL,*group_column="Group" ;
   char *condition_column="Condition",*contrast_name0=NULL,*contrast_name1=NULL ;
   char *pair_prefix=NULL ;
   char *censor_name=NULL,*atlas_name=NULL,*roi_sel=NULL,*matrix_name=NULL ;
   THD_datatable *tab=NULL ; THD_datatable_index *cindex=NULL ;
   THD_3dim_dataset **dset=NULL,*first=NULL,*mset=NULL,*out=NULL,*pairout=NULL ;
   byte *mask=NULL,*infer_valid=NULL,*temporal_valid=NULL,*condition_valid=NULL ;
   int *rowmap=NULL,*group=NULL,*gcount=NULL,**gmember=NULL ;
   char **glabel=NULL,**clabel=NULL ; THD_resample_set *bset=NULL ;
   PERM_scheme *pscheme=NULL,*cscheme=NULL ; PERM_set *pset=NULL,*cset=NULL ;
   PERM_result *presult=NULL,*cresult=NULL,*tresult=NULL ;
   THD_timeshift_set *tset=NULL ; THD_phase_set *phset=NULL ;
   unsigned char *tsneed=NULL ;
   int nsub,ngroup=0,ncond=1,ndset,ntime,ntime_input,nvox,nout,icol_group ;
   int ii,jj,vv,kk,cc,nmask,validtr_slot=-1 ;
   int *time_index=NULL ; byte *censor_keep=NULL ;
   int diff_slot=-1,boot_slot=-1,perm_slot=-1,temporal_slot=-1 ;
   int contrast_slot=-1,condperm_slot=-1,neffect=0 ;
   float **outval=NULL,*qval=NULL,*temporal_qval=NULL,*condition_qval=NULL ;
   float *group_max_neg=NULL ;
   float *pairval=NULL ; size_t nedge=0,npairmap=0 ;
   long long invalid_total=0,boot_invalid_total=0,temporal_invalid_total=0 ;
   ...
   if( !THD_ok_overwrite() ){
     ...
     if( pair_prefix!=NULL ){
       ...
       if( ncond>1 ) for( ii=0 ; ii<ncond ; ii++ ) for( jj=0 ; jj<ncond ; jj++ ){
         char ca[64],cb[64] ; insync_safe_token(ca,sizeof(ca),clabel[ii]) ;
   ...
   for( kk=0 ; kk<ngroup ; kk++ ){
     int pos=0 ; gmember[kk]=(int *)malloc(sizeof(int)*(size_t)gcount[kk]) ;
   ...
   for( ii=0 ; ii<nsub ; ii++ ) for( cc=0 ; cc<ncond ; cc++ ){
     int dd=ii*ncond+cc,rr=rowmap[dd] ; const char *who=cindex?cindex->level[0][ii]:tab->subj[ii] ;
   ...
   if( censor_name!=NULL ){
     MRI_IMAGE *cim=mri_read_1D(censor_name),*fim ; float *car ; int nkeep=0 ;
   ...
   if( temporal_mode==TNULL_TIMESHIFT ){
     ...
     if( method==METHOD_PAIRWISE ){
       ...
       for( ii=0 ; ii<nnull ; ii++ ){
         int aa,bb,pair=0,*off=tset->offset+(size_t)ii*nsub ;
         for( aa=0 ; aa<nsub ; aa++ ) for( bb=aa+1 ; bb<nsub ; bb++,pair++ ){
           int lag=(off[bb]-off[aa])%ntime ; if( lag<0 ) lag+=ntime ;
...

   {
     ...
     THD_memory_plan me ; double warn ;
   ...
   if( pset!=NULL ){
     int nelem=ncond*nvox ; infer_valid=(byte *)calloc((size_t)nelem,sizeof(byte)) ;
   ...
   if( tset!=NULL || phset!=NULL ){
     int nelem=ncond*ngroup*nvox ; temporal_valid=(byte *)calloc((size_t)nelem,sizeof(byte)) ;
     tresult=THD_perm_result_new(nelem,nnull) ; tresult->tail=temporal_tail ;
     for( ii=0 ; ii<nnull ; ii++ ) tresult->max_null[ii]=-FLT_MAX ;
   }
   if( cset!=NULL ){
     int nelem=neffect*nvox ; condition_valid=(byte *)calloc((size_t)nelem,sizeof(byte)) ;
   ...
   if( pair_prefix!=NULL ){
     size_t total ; npairmap=(size_t)ncond*ncond*nedge ;
...
#endif
   {
     int si,tt,gg,ci ; size_t nscratch=(nedge>(size_t)nsub)?nedge:(size_t)nsub ;
     ...
     int *sel0=(int *)malloc(sizeof(int)*(size_t)nsub) ; int *sel1=(int *)malloc(sizeof(int)*(size_t)nsub) ;
     byte *goodtime=(missing_policy==MISSING_COMMON)?(byte *)malloc((size_t)ntime):NULL ;
...
#pragma omp for schedule(dynamic,32)
     ...
     for( vv=0 ; vv<nvox ; vv++ ){
       ...
       for( ci=0 ; ci<ncond ; ci++ ){
         ...
         if( tresult!=NULL ){
           int ss,prep_good=!(method==METHOD_PAIRWISE && !pair_ok) ;
           ...
           for( gg=0 ; gg<ngroup ; gg++ ){
             int good=prep_good,e=(ci*ngroup+gg)*nvox+vv ; float obs=tnull[(size_t)gg*nnull] ;
             ...
             if( good ){
               ...
               for( ss=0 ; ss<nnull ; ss++ ){
                 float st=tnull[(size_t)gg*nnull+ss],cmp=(temporal_tail==PERM_TAIL_TWO)?fabsf(st):st ;
         ...
         if( pset!=NULL && isfinite(gstat[0]) && isfinite(gstat[1]) ){
           int ip,good=1,e=ci*nvox+vv ; float obs=gstat[0]-gstat[1] ;
           for( ip=0 ; ip<pset->nperm ; ip++ ){
             int pos0=0,pos1=gcount[0],*pp=pset->perm+(size_t)ip*nsub ; float st0,st1 ;
           ...
           if( cset!=NULL ){
             ...
             for( ip=0 ; ip<cset->nperm ; ip++ ){
               ...
               for( gg=0 ; gg<ngroup ; gg++ ){
                 int mm=gcount[gg] ; float a,b ;
                 for( si=0 ; si<mm ; si++ ){
                   int sj=gmember[gg][si],sw=(sgn[sj]<0) ; sel0[si]=sj+sw*nsub ; sel1[si]=sj+(1-sw)*nsub ;
                 }
                 if( method==METHOD_PAIRWISE ){
                   a=INSYNC_pairwise_indexed(2*nsub,csm->mat,mm,sel0,summary,scratch,NULL) ;
                   b=INSYNC_pairwise_indexed(2*nsub,csm->mat,mm,sel1,summary,scratch,NULL) ;
             ...
             for( e=0 ; e<neffect ; e++ ){
               int good=allgood,elem=e*nvox+vv ; float obs=ceffect[e] ;
               if( !isfinite(obs) ) good=0 ;
               if( good ){
...
#pragma omp critical(insync_condition_max)
   ...
   if( bad_data!=LLONG_MAX ){
     long long q=bad_data ; int ot=(int)(q%ntime_input),dd,ci,si,rr ;
   ...
   if( presult!=NULL ){
     int nelem=ncond*nvox,nvalid=0 ; for( ii=0 ; ii<nelem ; ii++ ) if( infer_valid[ii] ) nvalid++ ;
     ...
     for( cc=0 ; cc<ncond ; cc++ ){
       size_t off=(size_t)cc*nvox,nb=sizeof(float)*(size_t)nvox ; int bs=perm_slot+5*cc ;
   ...
   if( tresult!=NULL ){
     int nelem=ncond*ngroup*nvox,nvalid=0 ; for( ii=0 ; ii<nelem ; ii++ ) if( temporal_valid[ii] ) nvalid++ ;
     ...
     for( cc=0 ; cc<ncond ; cc++ ) for( kk=0 ; kk<ngroup ; kk++ ){
       size_t off=(size_t)(cc*ngroup+kk)*nvox,nb=sizeof(float)*(size_t)nvox ; int bs=temporal_slot+5*(cc*ngroup+kk) ;
   ...
   if( cresult!=NULL ){
     int nelem=neffect*nvox,nvalid=0 ; for( ii=0 ; ii<nelem ; ii++ ) if( condition_valid[ii] ) nvalid++ ;
     ...
     for( kk=0 ; kk<neffect ; kk++ ){
       size_t off=(size_t)kk*nvox,nb=sizeof(float)*(size_t)nvox ; int bs=condperm_slot+5*kk ;
   ...
   for( cc=0 ; cc<ncond ; cc++ ) for( kk=0 ; kk<ngroup ; kk++ ){
     char who[256],label[64] ; int bs=2*(cc*ngroup+kk) ;
   ...
   if( diff_slot>=0 ) for( cc=0 ; cc<ncond ; cc++ ){
     char who[256],label[64],both[160] ; snprintf(both,sizeof(both),"%s-%s",glabel[0],glabel[1]) ;
   ...
   if( nboot>0 ) for( cc=0 ; cc<ncond ; cc++ ) for( kk=0 ; kk<ngroup ; kk++ ){
     char who[256],label[64] ; int bs=boot_slot+2*(cc*ngroup+kk) ;
     if( ncond>1 ) snprintf(who,sizeof(who),"%s_%s",clabel[cc],glabel[kk]); else snprintf(who,sizeof(who),"%s",glabel[kk]);
   ...
   if( pset!=NULL ) for( cc=0 ; cc<ncond ; cc++ ){
     static char *stem[5]={"P","Z","Q","PFWE","ZFWE"}; char who[256],label[64] ; int bs=perm_slot+5*cc ;
   ...
   if( tresult!=NULL ) for( cc=0 ; cc<ncond ; cc++ ) for( kk=0 ; kk<ngroup ; kk++ ){
     static char *stem[5]={"TP","TZ","TQ","TPFWE","TZFWE"}; char who[256],label[64] ; int jj2,bs=temporal_slot+5*(cc*ngroup+kk) ;
     if( ncond>1 ) snprintf(who,sizeof(who),"%s_%s",clabel[cc],glabel[kk]); else snprintf(who,sizeof(who),"%s",glabel[kk]);
   ...
   if( have_contrast ) for( kk=0 ; kk<neffect ; kk++ ){
     ...
     if( cset!=NULL ){
       static char *stem[5]={"CP","CZ","CQ","CPFWE","CZFWE"}; int jj2,bs=condperm_slot+5*kk ;
       for( jj2=0 ; jj2<5 ; jj2++ ){ insync_brick_label(label,sizeof(label),stem[jj2],who); EDIT_substitute_brick(out,bs+jj2,MRI_float,outval[bs+jj2]); EDIT_BRICK_LABEL(out,bs+jj2,label); if(jj2==1||jj2==4)EDIT_BRICK_TO_FIZT(out,bs+jj2);else EDIT_BRICK_TO_NOSTAT(out,bs+jj2); }
     }
   }
   if( validtr_slot>=0 ){
   ...
   if( pairval!=NULL ){
     FILE *fp ; char table_name[THD_MAX_NAME],dataset_name[THD_MAX_NAME] ; size_t pm=0 ;
     ...
     for( cc=0 ; cc<ncond ; cc++ ) for( jj=0 ; jj<ncond ; jj++ ){
       ...
       for( aa=0 ; aa<nsub ; aa++ ) for( bb=aa+1 ; bb<nsub ; bb++,pair++,pm++ ){
         char la[64],lb[64],lc0[64],lc1[64],label[64],who[256] ;
         const char *sa=cindex?cindex->level[0][aa]:tab->subj[aa],*sb=cindex?cindex->level[0][bb]:tab->subj[bb] ;
