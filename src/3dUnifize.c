...
int main( int argc , char *argv[] )
{
   ...
   if( argc < 2 || strcmp(argv[1],"-help") == 0 ){
     ...
     printf("\n"
      "---------------------------------------------------\n"
      "HOW IT WORKS (Ziad's sneaky trick revealed at last)\n"
      "---------------------------------------------------\n"
      ...
      "via the '-clfrac' option.\n"
      "\n"
      "The second step is to scale every voxel x in the input volume by the\n"
      "factor 1000/WMI(x), so that the 'white matter intensity' is now\n"
      "uniform-ized to be 1000 everywhere.  (This is Ziad's 'trick'; it is\n"
      "easy, works well, and doesn't require fitting some model to the data.)\n"
      "\n"
      ...
      "'reflected' below 1000 to Q = 1000 - 2*(P-1000).  A lower bound for gray\n"
      ...
      "of all values between R and Q is computed; call this value G.  Then the\n"
      "values in the entire volume is linearly scaled by the formula\n"
      "   z_out = (1000-666)/(1000-G) * (z_in-1000) + 1000\n"
      "so that the WM average intensity of 1000 remains at 1000, and the gray\n"
      "matter median intensity of G is mapped to 666.  (Values z_out that end up\n"
      "negative are set to 0; as a result, much of CSF might end up as 0.)\n"
      "The value 666 was chosen becaue it gave results visually comparable to\n"
      "various 3 Tesla datasets.\n"
      "\n"
      "-----** CAVEAT **-----\n"
