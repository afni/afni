...
void BSY_help_the_pitiful_user(void)
{
  printf(
   ...
   "                    ++ As usual in AFNI, since the computation herein are\n"
   ...
   "   (just a 'good' permutation -- it is the best I could code quickly :).\n"
   "\n"
   "   Algorithm currently implemented (let A=BC' for notational simplicity):\n"
   "    1) Find the largest element A(i,j) in the matrix.\n"
   "       Then the permutation at row i is p(i)=j.\n"
   "       Strike row i and column j out of the matrix A.\n"
   "    2) Repeat, finding the largest element left, say at A(f,g).\n"
   ...
   "   and another step is used to improve it. For all pairs (i,j), p(i) and p(j)\n"
   "   are swapped and that permutation is tested to see if the trace gets bigger.\n"
   "   This pair-wise swapping is repeated until it doesn't improve things any\n"
   "   more (typically, it improves the trace about 1%% -- not much). The whole\n"
   "   permutation optimization procedure is very fast: about 1 second. In the\n"
   "   RS-FMRI data I've tried this on, the average time series permutation\n"
   "   resulting from this optimization is about 60%% of that which comes from\n"
   ...
   "   shows the trace BC' before any transforms, with the Q matrix transform,\n"
   ...
   "   the average time series correlation (in a gray matter mask) is about 63%%.\n"
   "\n"
   ...
   "   the run time (at least, for the cases I've tested).\n"
   "\n"
