...
void example_hierarchical(int nrows, int ncols, float** data, char* jobname, int k, float** distmatrix)
...
{ 
   ...
   */
   ...
   /* for double loop*/
   D = (float **)calloc(sizeof(float*), nrow-1);
   
   for (ii=0;ii<(nrow-1);++ii) {
    D[ii] = (float *)calloc(sizeof(float), ncol-1);
   }
