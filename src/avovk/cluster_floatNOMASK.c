...
static int
makedatamask(int nrows, int ncols, float*** pdata)
{ int i;
  float** data;
  int** mask;
...
static
float euclid (int n, float** data1, float** data2, 
...
static
float cityblock (int n, float** data1, float** data2, const float weight[], int index1, int index2, int transpose)
...
static
float correlation (int n, float** data1, float** data2, 
  const float weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
float acorrelation (int n, float** data1, float** data2, const float weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
float ucorrelation (int n, float** data1, float** data2, 
   const float weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
float uacorrelation (int n, float** data1, float** data2, 
  const float weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
float spearman (int n, float** data1, float** data2, 
   const float weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
float kendall (int n, float** data1, float** data2, 
  const float weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
{ int i, j, k;
  int** voxlcounter;
  voxlcounter = malloc(nclusters*sizeof(int*));
  for (i = 0; i < nclusters; i++)
    { voxlcounter[i] = malloc(ncolumns*sizeof(int));
    }
  ...
  { for (i = 0; i < nclusters; i++)
   ...
   for (i = 0; i < nclusters; i++)
    { for (j = 0; j < ncolumns; j++)
      { if (voxlcounter[i][j]>0)
        { cdata[i][j] /= voxlcounter[i][j];
          voxlcounter[i][j] = 1;
        }
      }
    } 
  ...
  free(voxlcounter);
  /* It might be good to free it later?! but it is small anyway... */
}
...
{ switch(method)
  { case 'm':
    { const int nelements = (transpose==0) ? nrows : ncolumns;
      ...
      if (!cache) return 0;
      getclustermedians(nclusters, nrows, ncolumns, data, clusterid,
                        cdata, transpose, cache);
      free(cache);
      return 1;
    }
    case 'a':
    { getclustermeans(nclusters, nrows, ncolumns, data, clusterid,
                      cdata, transpose);
      return 1;
    }
  }
...
static int
kmeans(int nclusters, int nrows, int ncolumns, float** data, 
  float weight[], int transpose, int npass, char dist,
  float** cdata, int clusterid[], float* error,
   ...
   if (npass!=0) randomassign (nclusters, nelements, tclusterid);
    ...
    { float previous = total;
      ...
      { float distance;
        ...
        distance = metric(ndata,data,cdata,weight,i,k,transpose);
        for (j = 0; j < nclusters; j++)
        { float tdistance;
          if (j==k) continue;
...
static int
kmedians(int nclusters, int nrows, int ncolumns, float** data, 
  float weight[], int transpose, int npass, char dist,
  float** cdata, int clusterid[], float* error,
  ...
  *error = DBL_MAX;
  ...
  { float total = DBL_MAX;
    ...
    { float previous = total;
      ...
      /* Find the center */
      getclustermedians(nclusters, nrows, ncolumns, data, tclusterid,
                        cdata, transpose, cache);
...
void CALL kcluster (int nclusters, int nrows, int ncolumns,
  float** data, float weight[], int transpose,
...
{ const int nelements = (transpose==0) ? nrows : ncolumns;
  ...
  *ifound = -1;
  ...
  else ok = makedatamask(ndata, nclusters, &cdata);
  if(!ok)
  { free(counts);
    if(npass>1)
    { free(tclusterid);
...
void CALL kmedoids (int nclusters, int nelements, float** distmatrix,
...
float** CALL distancematrix (int nrows, int ncolumns, float** data,
   float weights[], char dist, int transpose)
/*
Purpose
=======
...
{ int i,j;
  ...
  { result[i] += 1.0;
    for (j = 0; j < i; j++)
    { const float distance = metric(ndata, data, data, weights,
                                     i, j, transpose);
      if (distance < cutoff)
...
static
Node* pclcluster (int nrows, int ncolumns, float** data, 
...
{ int i, j;
  ...
  { /* Find the pair with the shortest distance */
    ...
    distid[js] = -inode-1;
    for (i = 0; i < js; i++)
      distmatrix[js][i] = metric(ndata,data,data,weight,js,i,0);
    for (i = js + 1; i < nnodes-inode; i++)
      distmatrix[i][js] = metric(ndata,data,data,weight,js,i,0);
  }
...
{ int i, j, k;
  ...
  { const int ndata = transpose ? nrows : ncolumns;
    ...
    { for (j = 0; j < i; j++) temp[j] =
        metric(ndata, data, data, weight, i, j, transpose);
      for (j = 0; j < i; j++)
      { k = vector[j];
        if (result[j].distance >= temp[j])
        { if (result[j].distance < temp[k]) temp[k] = result[j].distance;
...
{ Node* result = NULL;
  ...
  if(ldistmatrix)
  { distmatrix =
      distancematrix(nrows, ncolumns, data, weight, dist, transpose);
    if (!distmatrix) return NULL; /* Insufficient memory */
  }

  switch(method)
  { case 's':
      result = pslcluster(nrows, ncolumns, data, weight, distmatrix,
                          dist, transpose);
      break;
    case 'm':
    ...
    case 'a':
      ...
      break;
    case 'c':
      result = pclcluster(nrows, ncolumns, data, weight, distmatrix,
                          dist, transpose);
      break;
  }
...
float CALL clusterdistance (int nrows, int ncolumns, float** data,
   float weight[], int n1, int n2, int index1[], int index2[],
    ...
    { int i1, i2, j1, j2;
      ...
      for (i1 = 0; i1 < n1; i1++)
        ...
        { float distance;
          j1 = index1[i1];
          j2 = index2[i2];
          distance = metric (n,data,data,weight,j1,j2,transpose);
