...
------------------------------------------
The purpose of this script is to extract the data table from history of data
sets from 3dttest++, 3dMVM, or 3dLME.

Caveats:  (something more helpful coming later)

Statistics image must be DIRECTLY from 3dttest++, 3dMVM or 3dLME.
If you did 3dcopy or anything that wipes the history of the data set after
running the stats, this program has nothing to extract.
3dttest++ must have been run with no covariates.

------------------------------------------

Outputs files named with your -prefix:
(as example -prefix disco)

disco_GroupTable.csv:
    Table with information parsed from the statistics data set history.
    May include subject ID, any group or other variables, and input data sets.
...
optional.add_argument('-session',type=str,default="./",
                      help=("Output parent folder if you don't want the current working directory. [./]"))
