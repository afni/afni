                                 ...
                                 const int64_t * blist, nifti_brick_list * NBL)
{
   ...
   if( blist && nbricks <= 0 ){
      fprintf(stderr,"** nifti_image_read_bricks: bad nbricks, %ld\n", nbricks);
      return NULL;
   }
                                               ...
                                               int64_t nbricks )
{
   ...
   if( g_opts.debug > 2 ){
      fprintf(stderr,"+d updating image dimensions for %ld bricks in list\n",
              nbricks);
      fprintf(stderr,"   ndim = %ld\n",nim->ndim);
      fprintf(stderr,"   nx,ny,nz,nt,nu,nv,nw: (%ld,%ld,%ld,%ld,%ld,%ld,%ld)\n",
...
int nifti_update_dims_from_array( nifti_image * nim )
{
   ...
   if( g_opts.debug > 2 ){
      ...
      for( c = 0; c < 8; c++ ) fprintf(stderr," %ld", nim->dim[c]);
      fputc('\n',stderr);
   }
                             ...
                             const int64_t * blist, nifti_brick_list * NBL )
{
   ...
   if( blist && nbricks <= 0 ){
      if( g_opts.debug > 1 )
         fprintf(stderr,"-d load_bricks: received blist with nbricks = %ld,"
                        "ignoring blist\n", nbricks);
                        ...
                        int64_t * sindex, nifti_brick_list * NBL, znzFile fp )
{
   ...
   if( !slist ){
      for( c = 0; c < NBL->nbricks; c++ ) {
         ...
         if( rv != NBL->bsize ){
            fprintf(stderr,"** load bricks: cannot read brick %ld from '%s'\n",
                    c, nim->iname ? nim->iname : nim->fname);
            return -1;
         }
      }
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d read %ld default %ld-byte bricks from file %s\n",
       ...
       if( isrc != prev ){
          ...
          if( fposn != (oposn + isrc*NBL->bsize) ){
             ...
             if( znzseek(fp, fposn, SEEK_SET) < 0 ){
                fprintf(stderr,"** failed to locate brick %ld in file '%s'\n",
          ...
          if( rv != NBL->bsize ){
             fprintf(stderr,"** failed to read brick %ld from file '%s'\n",
                     isrc, nim->iname ? nim->iname : nim->fname);
             if( g_opts.debug > 1 )
                fprintf(stderr,"   (read %ld of %ld bytes)\n", rv, NBL->bsize);
             return -1;
          }
          fposn += NBL->bsize;
       } else {
                               ...
                               nifti_brick_list * nbl)
{
   ...
   if( ! nbl->bricks ){
      fprintf(stderr,"** NANM: failed to alloc %ld void ptrs\n",nbricks);
      return -1;
   }
   ...
   for( c = 0; c < nbl->nbricks; c++ ){
      ...
      if( ! nbl->bricks[c] ){
         fprintf(stderr,"** NANM: failed to alloc %ld bytes for brick %ld\n",
                 nbl->bsize, c);
   ...
   if( g_opts.debug > 2 )
      fprintf(stderr,"+d NANM: alloc'd %ld bricks of %ld bytes for NBL\n",
              nbl->nbricks, nbl->bsize);
                           ...
                           int64_t ** slist, int64_t ** sindex)
{
   ...
   *sindex = (int64_t *)malloc(nbricks * sizeof(int64_t));
   ...
   if( !*slist || !*sindex ){
      fprintf(stderr,"** NCS: failed to alloc %ld ints for sorting\n",nbricks);
   ...
   if( g_opts.debug > 2 ){
      ...
      fprintf(stderr,  "  orig   : ");
      for( c1 = 0; c1 < nbricks; c1++ ) fprintf(stderr,"  %ld",blist[c1]);
      fprintf(stderr,"\n  new    : ");
      for( c1 = 0; c1 < nbricks; c1++ ) fprintf(stderr,"  %ld",stmp[c1]);
      fprintf(stderr,"\n  indices: ");
      for( c1 = 0; c1 < nbricks; c1++ ) fprintf(stderr,"  %ld",itmp[c1]);
      fputc('\n', stderr);
   }
                           ...
                           const int64_t * blist, int disp_error)
{
   ...
   if( nim->dim[0] < 3 ){
      if( disp_error || g_opts.debug > 1 )
        fprintf(stderr,"** cannot read explict brick list from %ld-D dataset\n",
                nim->dim[0]);
      return 0;
   }
   ...
   if( nsubs <= 0 ){
      fprintf(stderr,"** VNBL warning: bad dim list (%ld,%ld,%ld,%ld)\n",
   ...
   for( c = 0; c < nbricks; c++ )
      if( (blist[c] < 0) || (blist[c] >= nsubs) ){
         if( disp_error || g_opts.debug > 1 )
            fprintf(stderr,
               "** volume index %ld (#%ld) is out of range [0,%ld]\n",
               blist[c], c, nsubs-1);
         return 0;
      }
                                 ...
                                 const nifti_brick_list *NBL)
{
   ...
   if( volbytes != NBL->bsize ) {
      if( g_opts.debug > 1 )
         fprintf(stderr,"** NBL/nim mismatch, volbytes = %ld, %ld\n",
                 NBL->bsize, volbytes);
      errs++;
   }
   ...
   if( nvols != NBL->nbricks ) {
      if( g_opts.debug > 1 )
         fprintf(stderr,"** NBL/nim mismatch, nvols = %ld, %ld\n",
                 NBL->nbricks, nvols);
      errs++;
   }
   ...
   if( errs ) return 0;
      ...
      fprintf(stderr,"-- nim/NBL agree: nvols = %ld, nbytes = %ld\n",
              nvols, volbytes);
...
char *nifti_strdup(const char *str)
{
  ...
  else      fprintf(stderr,"** nifti_strdup: failed to alloc %ld bytes\n",
                    strlen(str)+1);
...
int disp_nifti_2_header( const char * info, const nifti_2_header * hp )
{
   ...
   fprintf(fp,"    bitpix         = %d\n", hp->bitpix);
   fprintf(fp, "    dim[8]         =");
   for ( c = 0; c < 8; c++ ) fprintf(fp," %ld", hp->dim[c]);
   fputc('\n', fp);
   ...
   fprintf(fp, "    slice_start    = %ld\n", hp->slice_start);
   fprintf(fp, "    slice_end      = %ld\n", hp->slice_end);
...
int nifti_hdr2_looks_good(const nifti_2_header * hdr)
{
   ...
   d0 = hdr->dim[0];
   if( d0 < 0 || d0 > 7 ) {
      if( g_opts.debug > 0 ) fprintf(stderr,"** bad n2hdr: dim0 = %ld\n", d0);
      errs++;
...
static int nifti_read_extensions( nifti_image *nim, znzFile fp, int64_t remain )
{
   ...
   if( g_opts.debug > 2 )
      fprintf(stderr,"-d nre: posn=%ld, offset=%ld, type=%d, remain=%ld\n",
   ...
   if( remain < 16 ){
      if( g_opts.debug > 2 ){
         if( g_opts.skip_blank_ext )
            ...
            fprintf(stderr,"-d remain=%ld, no space for extensions\n",remain);
      }
      return 0;
   }
   ...
   remain -= 4;
   if( g_opts.debug > 2 )
      fprintf(stderr,"-d found valid 4-byte extender, remain = %ld\n", remain);
...
static znzFile nifti_image_load_prep( nifti_image *nim )
{
   ...
   if( znzseek(fp , (long)ioff , SEEK_SET) < 0 ){
      fprintf(stderr,"** could not seek to offset %ld in file '%s'\n",
              ioff, nim->iname);
      znzclose(fp);
      return NULL;
   }
  ...
  if( ii < ntot ){
    if( g_opts.debug > 0 )
       fprintf(stderr,"++ WARNING: nifti_read_buffer(%s):\n"
               "   data bytes needed = %ld\n"
               "   data bytes input  = %ld\n"
               "   number missing    = %ld (set to 0)\n",
                         ...
                         const nifti_brick_list * NBL)
{
   ...
   if( !NBL ){ /* just write one buffer and get out of here */
      ...
      if (ss < nim->nbyper * nim->nvox){
         fprintf(stderr,
            "** ERROR: NWAD: wrote only %ld of %ld bytes to file\n",
      ...
      if( ! NBL->bricks || NBL->nbricks <= 0 || NBL->bsize <= 0 ){
         fprintf(stderr,"** NWAD: no brick data to write (%p,%ld,%ld)\n",
      ...
      for( bnum = 0; bnum < NBL->nbricks; bnum++ ){
         ...
         if( ss < NBL->bsize ){
            fprintf(stderr,
            "** NWAD ERROR: wrote %ld of %ld bytes of brick %ld of %ld to file",
            ss, NBL->bsize, bnum+1, NBL->nbricks);
            return -1;
         }
      }
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d wrote image of %ld brick(s), each of %ld bytes\n",
                 NBL->nbricks, NBL->bsize);
   }
                                          ...
                                          int arg_dtype)
{
   ...
   if( dim[0] < 1 || dim[0] > 7 ) {
      fprintf(stderr,"** nifti_simple_hdr_with_dims: bad dim[0]=%ld\n",dim[0]);
      dim = default_dims;
   } else {
      for( c = 1; c <= dim[0]; c++ )
         if( dim[c] < 1 )
         {
            fprintf(stderr,
                "** nifti_simple_hdr_with_dims: bad dim[%d]=%ld\n",c,dim[c]);
            dim = default_dims;
            break;
         }
   }
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"+d make_new_n2_header, dim[0] = %ld, datatype = %d\n",
              dim[0], dtype);
                                          ...
                                          int arg_dtype)
{
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"+d make_new_n1_header, dim[0] = %ld, datatype = %d\n",
              dim[0], dtype);
                                 ...
                                 int data_fill)
{
   ...
   if( data_fill ) {
      ...
      if( !nim->data ) {
         fprintf(stderr,"** NMNN: failed to alloc %ld bytes for data\n",
                 nim->nvox*nim->nbyper);
         nifti_image_free(nim);
         nim = NULL;
      }
   ...
   switch( nim->nifti_type ){
     ...
     case NIFTI_FTYPE_NIFTI1_1:
       ...
       if( nim->iname_offset != offset ){
          if( g_opts.debug > 1 )
             fprintf(stderr,"+d changing offset from %ld to %ld\n",
               ...
               const char * opts, znzFile imgfile, const nifti_brick_list * NBL)
{
   ...
   if( g_opts.debug > 1 ){
      ...
      if( g_opts.debug > 2 )
         fprintf(stderr,"-d nifti type %d, offset %ld\n",
...
char *nifti_image_to_ascii( const nifti_image *nim )
{
   ...
   sprintf( buf+strlen(buf) , "  image_offset = '%ld'\n" , nim->iname_offset );

                       sprintf( buf+strlen(buf), "  ndim = '%ld'\n",nim->ndim);
                       sprintf( buf+strlen(buf), "  nx = '%ld'\n",  nim->nx  );
   if( nim->ndim > 1 ) sprintf( buf+strlen(buf), "  ny = '%ld'\n",  nim->ny  );
   if( nim->ndim > 2 ) sprintf( buf+strlen(buf), "  nz = '%ld'\n",  nim->nz  );
   if( nim->ndim > 3 ) sprintf( buf+strlen(buf), "  nt = '%ld'\n",  nim->nt  );
   if( nim->ndim > 4 ) sprintf( buf+strlen(buf), "  nu = '%ld'\n",  nim->nu  );
   if( nim->ndim > 5 ) sprintf( buf+strlen(buf), "  nv = '%ld'\n",  nim->nv  );
   if( nim->ndim > 6 ) sprintf( buf+strlen(buf), "  nw = '%ld'\n",  nim->nw  );
...
int nifti_nim_has_valid_dims(nifti_image * nim, int complain)
{
   ...
   if( nim->dim[0] <= 0 || nim->dim[0] > 7 ){
      errs++;
      if( complain )
        fprintf(stderr,"** NVd: dim[0] (%ld) out of range [1,7]\n",nim->dim[0]);
      return 0;
   }
   ...
   if( nim->ndim != nim->dim[0] ){
      errs++;
      if( ! complain ) return 0;
      fprintf(stderr,"** NVd: ndim != dim[0] (%ld,%ld)\n",
              nim->ndim,nim->dim[0]);
   }
   ...
   if( ( (nim->dim[0] >= 1) && (nim->dim[1] != nim->nx) ) ||
      ...
      errs++;
      if( !complain ) return 0;
      fprintf(stderr,"** NVd mismatch: dims    = %ld,%ld,%ld,%ld,%ld,%ld,%ld\n"
                     "                 nxyz... = %ld,%ld,%ld,%ld,%ld,%ld,%ld\n",
   ...
   if( g_opts.debug > 2 ){
      fprintf(stderr,"-d check dim[%ld] =", nim->dim[0]);
      for( c = 0; c < 7; c++ ) fprintf(stderr," %ld", nim->dim[c]);
      fputc('\n', stderr);
   }
   ...
   for( c = 1; c <= nim->dim[0]; c++ ){
      if( nim->dim[c] > 0)
         ...
         if( !complain ) return 0;
         fprintf(stderr,"** NVd: dim[%ld] (=%ld) <= 0\n",c, nim->dim[c]);
         errs++;
      }
   }
   if( prod != nim->nvox ){
      if( ! complain ) return 0;
      fprintf(stderr,"** NVd: nvox does not match %ld-dim product (%ld, %ld)\n",
   ...
   if( g_opts.debug > 1 )
      for( c = nim->dim[0]+1; c <= 7; c++ )
         if( nim->dim[c] != 0 && nim->dim[c] != 1 )
            fprintf(stderr,"** NVd warning: dim[%ld] = %ld, but ndim = %ld\n",
                                ...
                                void ** data )
{
   ...
   if( g_opts.debug > 2 ){
      ...
      for(c = 0; c < 8; c++) fprintf(stderr," %3ld", dims[c]);
      fprintf(stderr,"\n   nim->dims =");
      for(c = 0; c < 8; c++) fprintf(stderr," %3ld", nim->dim[c]);
      fputc('\n', stderr);
   }
   ...
   for( c = 1; c <= nim->dim[0]; c++ ){
      if( dims[c] >= nim->dim[c] ){
         fprintf(stderr,"** nifti_RCI: dims[%ld] >= nim->dim[%ld] (%ld,%ld)\n",
                         ...
                         znzFile fp, int64_t base_offset)
{
   ...
   if( nprods == 1 ){
      ...
      if( nread != bytes ){
         ...
         fprintf(stderr,"+d successful read of %ld bytes at offset %ld\n",
                 bytes, base_offset);
   ...
   for( c = 0; c < prods[0]; c++ ){
      ...
      if( g_opts.debug > 3 )
         fprintf(stderr,"-d reading %ld bytes, foff %ld + %ld, doff %ld\n",
...
static int rci_alloc_mem(void **data, int64_t prods[8], int nprods, int nbyper )
{
   ...
   if( ! *data ){   /* then allocate what is needed */
      ...
      *data = malloc(size);   /* actually allocate the memory */
      ...
      fprintf(stderr,"-d rci_am: *data already set, need %ld x %d bytes\n",
              size/nbyper, nbyper);
                                             ...
                                             int64_t prods[], int * nprods )
{
   ...
   *nprods = len;
   ...
   if( g_opts.debug > 2 ){
      ...
      for(dind = 0; dind < len; dind++) fprintf(stderr," %ld", prods[dind]);
      fputc('\n',stderr);
   }
...
int64_t * nifti_get_int64list( int64_t nvals , const char * str )
{
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"-d making int_list (vals = %ld) from '%s'\n", nvals, str);
   ...
   while( ipos < slen && !ISEND(str[ipos]) ){
     ...
     while( isspace((int) str[ipos]) ) ipos++ ;   /* skip blanks */
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( ibot < 0 ){
           fprintf(stderr,"** ERROR: list index %ld is out of range 0..%ld\n",
                   ibot,nvals-1) ;
      ...
      if( str[ipos] == ',' || ISEND(str[ipos]) ){
         ...
         if( !subv ) {
            fprintf(stderr,"** nifti_get_intlist: failed realloc of %ld ints\n",
                    nout+1);
            return NULL;
         }
         subv[0]    = nout ;
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( itop < 0 ){
           fprintf(stderr,"** ERROR: index %ld is out of range 0..%ld\n",
                   itop,nvals-1) ;
           free(subv) ; return NULL ;
         }
         if( itop >= nvals ){
   ...
   if( g_opts.debug > 1 ) {
      fprintf(stderr,"+d int_list (vals = %ld): ", subv[0]);
      for( ii = 1; ii <= subv[0]; ii++ ) fprintf(stderr,"%ld ", subv[ii]);
      fputc('\n',stderr);
   }
...
int * nifti_get_intlist( int nvals , const char * str )
{
   ...
   if( nints > INT_MAX ) {
      fprintf(stderr,"** N_get_intlist: %ld ints is too long for 32-bits\n",
              nints);
      free(i64list);
      return NULL;
   }
   ...
   if( !ilist ) {
      fprintf(stderr,"** N_get_intlist: failed to alloc %ld ints\n", nints);
      free(i64list);
      return NULL;
   }
   ...
   for( index=0; index <= nints; index++ ) {
      if( i64list[index] > INT_MAX ) {
         fprintf(stderr,"** N_get_intlist: value %ld too big for 32-bits\n",
                 i64list[index]);
         free(ilist);
         free(i64list);
         return NULL;
