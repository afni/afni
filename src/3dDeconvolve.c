...
typedef struct {
  ...
  float (*f)(float,float,float,float) ;  /* function: f(t,a,b,c) */
} basis_func ;

/*! Macro to evaluate a basis_func at a particular point. */

#define basis_funceval(bf,x) ((bf).f( (x), (bf).a,(bf).b,(bf).c ))
  ...
  *num_blocks = 0 ;  /* 04 Aug 2004 */
  ...
  if (option_data->nodata)
      ...
      if (!ISVALID_3DIM_DATASET(*dset_time))
	{
      ...
      if( DSET_TIMEUNITS(*dset_time) == UNITS_MSEC_TYPE ) basis_TR *= 0.001 ;
...
#undef  TPEAK
#define TPEAK(TT) ((TT)/(1.0l-exp(-0.2l*(TT))))

static float basis_block_hrf( float t, float T )
{
   float t2,t3,t4,t5,t6,t7,t9,t10,t11,t14,t20,t25,t28,t37,t57 , w ;

   if( t <= 0.0f || t >= (T+15.0f) ) return 0.0f ;
...
static float basis_block( float t , float T , float peak , float junk )
{
   float w , tp , pp ;

   w = basis_block_hrf(t,T) ;
   if( w > 0.0f && peak > 0.0f ){
     tp = TPEAK(T) ;  pp = basis_block_hrf(tp,T) ;
...
basis_expansion * basis_parser( char *sym )
{
   ...
   /*--- BLOCK(duration,peak) ---*/
     ...
     if( cpt == NULL ){
       ...
       fprintf(stderr,
        "          Correct format: 'BLOCK(dur)' with dur > 0,\n"
        "                     *OR* 'BLOCK(dur,peak)' with peak > 0 too.\n") ;
       free((void *)be); free(scp); return NULL;
     }
