...
/** Macros to change the parameters in a basis_func structure **/

#define basis_func_seta(bf,aa) (bf).a = (aa)
#define basis_func_setb(bf,bb) (bf).b = (bb)
#define basis_func_setc(bf,cc) (bf).c = (cc)
...
typedef struct {
  int type , nfunc , nparm , pbot , timetype , vfun,vmod ;
  float tbot,ttop ;
  basis_func *bfunc ;
...
ENTRY("read_input_data") ;
  ...
  *num_blocks = 0 ;  /* 04 Aug 2004 */
  ...
  if( basis_count > 0 ){             /* if there are any, that is */
    ...
    MRI_IMARR *vimar ; int vdim , vmod , vv ;
    ...
    for( is=0 ; is < num_stimts ; is++ ){
      ...
      if( tim->kind == MRI_fvect ){  /* modulation */
        ...
        vdim  = IMARR_COUNT(vimar) ; vmod = vdim - 1 ;
        for( vv=0 ; vv < vmod ; vv++ )
      ...
      for( vv=0 ; vv < vmod ; vv++ )  /* space to collect amplitudes */
      ...
      if( be->timetype == GLOBAL_TIMES ){  /****------ global times ------****/
        ...
        for( ii=0 ; ii < nx*ny ; ii++ ){ /* loop over all input times */
          ...
          if( tt >= 0.0f && tt <= tmax ){ /* a good time value */
            for( vv=0 ; vv < vmod ; vv++ ) zar[vv][ngood] = aar[vv][ii] ;
        ...
        for( jj=0 ; jj < ny ; jj++ ){   /* jj=row index=block index */
          ...
          for( nout=ii=0 ; ii < nx ; ii++ ){
            tt = tar[ii+jj*nx] ;
            if( tt >= 0.0f && tt <= tmax ){
              for( vv=0 ; vv < vmod ; vv++ ) zar[vv][ngood] = aar[vv][ii+jj*nx] ;
      ...
      if( ngood > 1 ){  /** check for duplicates [16 Aug 2007] **/
        ...
        for( ii=0 ; ii < ngood ; ii++ ){
          ...
          for( jj=ii+1 ; jj < ngood ; jj++ ){
            ...
            else if( dt < 0.05f ) nndup++ ;  /* only 5% of a TR apart */
          }
        }
        if( ndup > 0 || nndup > 0 ){
          WARNING_message(
      ...
      if( ngood == 0 ){    /* WTF? no good values found */
        ...
        for( vv=0 ; vv < vmod ; vv++ ) free(zar[vv]) ;
        badlev++ ;

      } else if( vmod == 0 ){  /* no paired values */
        ...
        for( vv=0 ; vv < vmod ; vv++ ){      /* add images of paired values */
