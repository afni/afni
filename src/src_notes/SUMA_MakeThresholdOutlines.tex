...
\begin{itemize}
	...
	\item \texttt{nd = {\bf SUMA\_GetNodeDef}(dset)}
	\begin{itemize}
												...
												\hspace*{0.9in} $\bigtriangleup$ {\bf SUMA\_Build\_Cluster\_From\_Node\_NoRec}\\
													...
													\hspace*{1.0in} $\bigtriangledown$ While nodes to be assigned and $\exists$ candidate list\\
														...
														\hspace*{1.2in} $\triangleleft$ search to see if any are to be assigned\\
															...
															\hspace*{1.4in} $\triangleright$ else (accept nodes connected by up to -((int)Opt->DistLim) edges)
																\hspace*{1.6in} $\cdot$ For each unassigned node ...\\
																	\hspace*{1.8in} $\longrightarrow$ take that node into the cluster\\
																	\hspace*{1.8in} $\longrightarrow$ mark it as assigned, an reduce the number of nodes left to assign\\
																	\hspace*{1.8in} $\longrightarrow$ mark it as a candidate if it has not been visited as a candidate before\\
															\hspace*{1.4in} $\triangleright$ {\bf SUMA\_Recycle\_getoffsets}\\
																\hspace*{1.6in} $\cdot$ For each layer, \\
																	\hspace*{1.8in} $\longrightarrow$ reset the layer index of used nodes in LayerVect\\
