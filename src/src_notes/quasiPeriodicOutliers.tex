...
{\em  Functional connectivity} typically refers to a high level of temporal correlation among spatially distinct locations within the brain.  Birn \cite{Birn2012} estimated functional connectivity among different brain regions from MRI data by computing the temporal correlation of low frequency (<0.1 Hz) ﬂuctuations in the MRI signal.  These fluctuations occurred even when the subject was ``at rest''.  Correlated signal ﬂuctuations occurred at low temporal frequencies (<0.1 Hz).  Biswal {\em et al.}\cite{Biswal1995} found that signal ﬂuctuations in the left and right primary motor areas were signiﬁcantly correlated even when the subject was not performing any motor task.  These correlated signal ﬂuctuations occurred at low temporal frequencies (<0.1 Hz)\cite{Birn2012}.  However, this requires accurate detection of peaks and troughs.
...
Liu {\em et al}\cite{Liu2022} propose a collection of algorithms of various levels of complexity with more complex algorithms having slightly better performance.  TCQSA also uses clustering but is conceptually simpler than Seq2GMM.   TCQSA first automatically splits the QTS into quasi-periods which are then classified by the {\em hybrid attentional Long Short-Term Memory-Convolutional Neural Network (LSTM-CNN) model} (HALCM) into normal periods or anomalies.  TCQSA integrates an hierarchical clustering and the k-means technique.  TCQSA contains two levels of clustering. Particularly, the former clustering adopts a hierarchical clustering, avoiding manual intervention.  The second clustering is speciﬁcally utilized to eliminate the clusters caused by the outliers of QTSs due to noise.  This may make the second clustering undesirable if the goal is to identify regions of badness.  TCQSA is aimed more at identifying medical anomalies such as arrhythmia.  However, the outliers, identified during the former clustering may correspond to badness regions.
...
For respiratory data, one should make a new set of peaks each of which is the difference between each peak value and the mean of the values of the troughs on either side.  There should be only one peak between two troughs and only one trough between two peaks.  If the peaks and troughs are not properly interdigitated, this problem should be addressed before the peak to trough calculation is made.  The range, between the two peaks is recorded in an output list where each nested member gives the range between the two peaks.  
...
\begin{enumerate}
	...
	\item Identify outliers on low end of the cumulative weights using a variation of the Tukey method \cite{Tukey1977}.
	\begin{enumerate}
	...
	\item Ensure peaks and troughs are {\em bijective}.  That is, every pair of peaks is separated by exactly one trough and vice versa.
	\begin{enumerate}
		\item Identify peak-trough mismatches (cases where a pair of peaks is not separated by exactly one trough).
		\item Merge the peak and trough arrays into a single chronologically sorted list of events, each tagged as either a peak (`P`) or a trough (`T`).
		\item Convert \texttt{respiratoryPeaks} and \texttt{respiratoryTroughs} to NumPy arrays for vectorized operations.
		\item For each consecutive pair of peaks $(p_1, p_2)$, find all troughs strictly between them; if exactly one such trough $t$ exists, record $(p_1, t, p_2)$ as a valid respiratory cycle.
		\item For each valid cycle, compute the peak-to-trough duration ($t - p_1$) and the trough-to-peak duration ($p_2 - t$).
		\item Compute the median fractional phase position of the trough within a cycle:
		\[
		\text{frac} = \operatorname{median}\left(\frac{t - p_1}{(t - p_1) + (p_2 - t)}\right)
		\]
		If no valid cycles were found, default to $\text{frac} = 0.5$ (trough assumed halfway between peaks).
		\item Re-sort all peaks and troughs together into a single chronological event list (recomputed, since the arrays haven't changed at this point).
		\item Scan the sorted events for consecutive pairs sharing the same type (two peaks in a row, or two troughs in a row) and collect these as \texttt{bad\_events}, indicating a bijectivity violation.
		\item For each such same-type adjacent pair $(x_1, x_2)$:
		\begin{enumerate}
			\item If both are peaks (`PP`), insert a missing trough at $x_1 + \text{frac} \cdot (x_2 - x_1)$, rounded to the nearest integer.
			\item If both are troughs (`TT`), insert a missing peak at $x_1 - \text{frac} \cdot (x_1 - x_2)$, rounded to the nearest integer.
		\end{enumerate}
		\item Append the newly inferred peaks to \texttt{respiratoryPeaks} and the newly inferred troughs to \texttt{respiratoryTroughs}, then sort each resulting array.
		\item Return the corrected peaks array, the corrected troughs array, and the lists of added peaks and added troughs.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item Normalize time differences to be in the same range as the cumulative weights.
...
Unfortunately, the variation from periodicity resulted in the new peaks being significantly displaced from where they should be.  In some cases, even the number of peaks was wrong.  Hence, the missing peaks are treated as a phase interpolation problem, rather than a periodic interpolation problem.  The procedure is as follows.
\begin{enumerate}
	\item Keep the last few valid RR (the time (or number of samples) between consecutive R peaks in the ECG) intervals before the bad region.
	\item Keep the first few valid RR intervals after the bad region.
	\item Estimate the missing number of beats from the surrounding rhythm.
	\item Place missing peaks by interpolating {\em phase} between the two known peaks.
\end{enumerate}

An example of the results is shown in figure \ref{fig:cardiacPhaseInterpolation}.

\begin{figure}[htb]
	\begin{center}
		\includegraphics[height=4.0in,width=3in,angle=0]{cardiacPhaseInterpolation.pdf}
		\caption{Effect of correcting cardiac peak locations using phase interpolation.  The blue spots are the original peaks in the good regions.  The red spots are the phase interpolated peaks in the bad regions.}
		\label{fig:cardiacPhaseInterpolation}
	\end{center}
\end{figure}
...
This places the peaks (and troughs) on a type of invisible clothesline which is preferable to trying to match the erroneous peaks (and troughs) of the input signal.  An optional modification would be to move each of the peaks (and troughs) to the local maximum (minimum), redetermine the bad segments and then recorrect with the standard clothesline method.

\subsection{Visualization}

Ideally, users would prefer to see the bad regions, and their corrections, against the original time series.  However, slow drift in the time series profile may be much greater than the amplitude of the quasi-periodic respiratory and cardiac activity.  This is particularly the case for the respiratory profile.  Hence, while viewing the results against the original time series is the default, there is the option to flatten the drift using ALS (asymmetric least squares) baseline correction\cite{Eilers2005}.  This estimates the ALS baseline of the time series (using the pybaselines:Baseline function alsBaseline) and subtracts it from the original time series.  An example of the results is shown in figure \ref{BaslineCorrection}.


\begin{figure}
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.6\linewidth]{beforeALSBaselineCorrection.pdf}
		\caption{Before baseline correction}
		\label{fig:sfig1}
	\end{subfigure}%
	\\
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.6\linewidth]{AlsBaselineCorrected.pdf}
		\caption{After baseline correction}
		\label{fig:sfig2}
	\end{subfigure}
	\caption{Corrected respiratory profiles (Dan Handwerker, sub 1, run 1) before (\ref{fig:sfig1}) and after  (\ref{fig:sfig2}) ALS baseline correction.  Note how removing the secular drift expands the visualized amplitude, making the results of correction easier to visualize.}
	\label{BaslineCorrection}
\end{figure}
...
\begin{thebibliography}{99} % Bibliography - this is intentionally simple in this template
	...
	\bibitem{Tukey1977} Tukey, J. W. (1977). {\em Exploratory Data Analysis}. Addison-Wesley. 
	...
	\bibitem{Reynolds2024} Reynolds RC, Glen DR, Chen G, Saad ZS, Cox RW, Taylor PA (2024). Processing, evaluating and understanding FMRI data with afni\_proc.py. {\em Imaging Neuroscience} 2:1-52
	
	\bibitem{Eilers2005} Eilers, Paul H. C., \& Boelens, Hans F.M., ``Baseline Correction with Asymmetric Least Squares Smoothing''. Leiden University Medical Centre, 2005
