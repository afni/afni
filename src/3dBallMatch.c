...
void Bmatch_help(void)
{
   printf("\n"
    ...
    "    set ijk = ( `cat $sub.match` )\n"
    "# set shift range to be 50 mm about 3dBallMatch coordinates\n"
    "    set  xd = $ijk[7] ; set xbot = `ccalc \"${xd}-50\"` ; set xtop = `ccalc \"${xd}+50\"`\n"
    "    set  yd = $ijk[8] ; set ybot = `ccalc \"${yd}-50\"` ; set ytop = `ccalc \"${yd}+50\"`\n"
    "    set  zd = $ijk[9] ; set zbot = `ccalc \"${zd}-50\"` ; set ztop = `ccalc \"${zd}+50\"`\n"
    "\n"
    ...
    "  im_to_mov -resize -prefix Bsag  -npure 4 -nfade 0 [1-9]*.sag.jpg\n"
    "  im_to_mov -resize -prefix ABsag -npure 4 -nfade 0 A[1-9]*.sag.jpg\n"
    "exit 0\n"
    "\n"
    "HOW IT WORKS\n"
    "------------\n"
