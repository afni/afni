...
set ac = 1
while ( $ac <= $#argv )
  set oshort = `echo $argv[$ac] | cut -b 1-4`
  if ( "$argv[$ac]" == "-help" ) then
    ...
    exit
  else if ( "$oshort" == "-bin" ) then
    @ ac += 1
    set bin_dir = $argv[$ac]
  else if ( "$oshort" == "-src" ) then
    @ ac += 1
    set src_dir = $argv[$ac]
  else
...
# ======================================================================
# perform any useful checks
if ( ! -d $bin_dir ) then
   echo "** missing -bin_dir: $bin_dir"
   exit 1
endif

if ( ! -d $src_dir ) then
   echo "** missing -src_dir: $src_dir"
   exit 1
endif
