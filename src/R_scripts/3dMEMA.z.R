...
read.MEMA.opts.interactive <- function () {
...
"What value will covariate(s) be centered around (0: mean; 1: other value)? "))
      ...
      if(lop$nGrp == 1) {
         ...
         if(lop$centerType == 1) {  # center around a user-specified value
            ...
            for(ii in 1:lop$nCov) centerVal[ii] <- as.numeric(readline(sprintf("Centering value for covariate no. %i (%s): ? ", ii, names(lop$covData)[ii])))
            lop$covData <- as.matrix(apply(lop$covData, 1, "-", centerVal))
         }
      } # if(lop$nGrp == 1)
      ...
      if (lop$nGrp == 2) {
         ...
         if(lop$centerType2 == 0 | lop$centerType2 == 1) {   # same center 
            ...
            if(lop$centerType == 1) {  
               ...
               centerVal <- vector(mode = "numeric", length = lop$nCov)
               for(jj in 1:lop$nCov) centerVal[jj] <- as.numeric(readline(sprintf(
            ...
            if(lop$centerType == 1) {
               ...
               for(ii in 1:lop$nGrp) {
                ...
                "Centering value for covariate no. %i (%s) in group %i (%s): ? ",
                  ...
                  covList[[ii]] <- t(apply(as.matrix(lop$covData[((ii-1)*lop$nSubj[1]+1):(lop$nSubj[1]+(ii-1)*lop$nSubj[2]),]), 1, "-", centerVal))
               }
...
read.MEMA.opts.batch <- function (args=NULL, verb = 0) {
   help_lst <- list (
      '-prefix' = 
         paste("Output file name (just prefix, no view+suffix needed) ",
               "e.g., myOutput: ", sep='\n'),
      '-jobs' = 
         paste("On a multi-processor machine, parallel computing will speed ",
   ...
   params <- list (
      ...
      '-equal_variance' = 0, 
      '-covariates' = 1,
      '-covariates_center' = c(1,Inf),
      '-covariates_model' = c(2),
      '-covariates_name' = c(1,Inf)
                     );
   ...
   for (i in 1:length(ops)) {
      ...
      switch(opname,
             ...
             outlier_Z = lop$resZout <- 1,
             no_outlier_Z = lop$resZout <- 0,
   ...
   if (!is.null(lop$covFN)) {
      ...
      if ( tolower(ttt$V1[1]) == 'name' || 
         ...
         for (ii in 1:(dim(ttt)[2])) { #Add one column at a time
            if (ii==1) {
               lop$covMatrix <- cbind(
                  as.numeric(ttt[1:dim(ttt)[1],1:dim(ttt)[2]][[ii]]));
            } else {
               lop$covMatrix <- cbind(lop$covMatrix,
                  as.numeric(ttt[1:dim(ttt)[1],1:dim(ttt)[2]][[ii]]));
            }
         }
         if (dim(ttt)[1] != length(allsubj)) {
            warning (paste('Have ', length(allsubj), ' subjects, ',
                           'but ', dim(ttt)[1], 'covariate values.\n',
...
process.MEMA.opts <- function (lop, verb = 0) {
{
   ...
   #lop$masked <- as.integer(readline("Any mask (0: no; 1: yes)? "))
   if(!is.null(lop$maskFN)) {
      #maskFN <- 
      #  readline("Mask file name (suffix unnecessary, e.g., mask+tlrc): "); 
      if (verb) {
   ...
   if(!is.null(lop$covFN)) {
#      lop$nCov <- as.integer(readline("Number of covariates (e.g., 1)? "))
#       lop$nCov  should be set after reading files
...
#      print("input files listed above.")
      ...
      #      } else {
      #         for (ii in 1:lop$nCov) 
      #            cat ('Need to take care of naming here...\n',
      #                  'See lop$covName below\n')
      #            #names(lop$covData)[ii] <- 
      #           #   readline(sprintf("Name for covariate number %i? ", ii))
      #            
      #      }
      ...
      if(lop$nGrp == 1) {
         ...
         if(lop$centerType == 1) {  # center around a user-specified value
            ...
            for(ii in 1:lop$nCov) {
               ...
               #centerVal[ii] <- as.numeric(readline(sprintf("Centering value for covariate no. %i (%s): ? ", ii, names(lop$covData)[ii])))
            }
            lop$covData <- as.matrix(apply(lop$covData, 1, "-", lop$centerVal))
         }
      } # if(lop$nGrp == 1)
         ...
         if(lop$centerType2 == 0 | lop$centerType2 == 1) {   # same center 
            ...
            if(lop$centerType == 1) {  
               ...
               for(jj in 1:lop$nCov) {
                  ...
                  #centerVal[jj] <- as.numeric(readline(sprintf(
            #"Centering value for covariate no. %i (%s) for both groups: ? ", 
            #                                       jj, names(lop$covData)[jj])))
               }
               lop$covData <- as.matrix(
         ...
         if(lop$centerType2 == 2 | lop$centerType2 == 3) {  # different center 
            ...
            if(lop$centerType == 1) {
               ...
               for(ii in 1:lop$nGrp) {
                  ...
                  #centerVal <- vector(mode = "numeric", length = lop$nCov)
                  #for(jj in 1:lop$nCov)  {
                  #   cat ('Have lop$centerVal[jj]', centerVal[jj], 
                   #        'group ',ii,'\n');
                     #centerVal[jj] <- 
                     #   as.numeric(readline(sprintf(
                #"Centering value for covariate no. %i (%s) in group %i (%s): ? ",
...
print("Use CNTL-C on Unix or ESC on GUI version of R to stop at any moment.")
   ...
   if (!length(args)) {
      ...
      str(lop);
      if (is.null(lop <- process.MEMA.opts(lop))) {
   ...
   if(lop$anaType==3) {
      for(ii in 1:lop$nLevel) {
      lop$bList[[ii]] <- lapply(lop$bList[[ii]], function(x) x$ttt); lop$tList[[ii]] <- lapply(lop$tList[[ii]], function(x) x$ttt)
      lop$varList[[ii]] <- mapply(function(x, y) ifelse((abs(x)<tolL) | (abs(y)<tolL), 0, (x/y)^2), lop$bList[[ii]], lop$tList[[ii]], SIMPLIFY = FALSE)  # variances
      if(exclude) for (jj in 1:lop$nSubj[1]) lop$varList[[ii]][[jj]][lop$varList[[ii]][[jj]] < tolL] <- tolU  # replace those 0 variances with a big number
      }
      contrBList <- mapply("-", lop$bList[[1]], lop$bList[[2]], SIMPLIFY = FALSE)
      contrVarList <- mapply("+", lop$varList[[1]], lop$varList[[2]], SIMPLIFY = FALSE)
   }
   ...
   #if(lop$anaType==4) nBrick  <- 4*lop$nGrp+(anyCov)*2*lop$nCov+2+2*sum(lop$nSubj)*lop$resZout else nBrick <- 4*lop$nGrp+(anyCov)*2*lop$nCov+2*sum(lop$nSubj)*lop$resZout
   ...
   nBrick0 <- 4*lop$nGrp+(anyCov)*2*lop$nCov   # no. sub-bricks in the main output
   nBrick <- 4*lop$nGrp+(anyCov)*2*lop$nCov+2*sum(lop$nSubj)*lop$resZout  # total sub-bricks in all output
   if(lop$anaType==4) {nBrick0 <- nBrick0+4; nBrick <- nBrick+4}  # two more for lop$anaType==4
      
   if(lop$anaType==1 | lop$anaType==2 | lop$anaType==4) comArr <- array(c(unlist(c(lop$bList)), unlist(c(lop$varList))), dim=c(lop$myDim[1:3], sum(lop$nFiles)))
   if(lop$anaType==3) comArr <- array(c(unlist(c(contrBList)), unlist(c(contrVarList))), dim=c(lop$myDim[1:3], sum(lop$nFiles)))

   #rm(lop$bList, lop$varList)
   lop$bList <- list();
   lop$varList <- list();
...
#   print("Starting to analyze data slice by slice...")
   ...
   for(ii in 1:lop$nGrp) {
      ...
      if(lop$KHtest) outLabel <- append(outLabel, sprintf("%s:t", lop$grpLab[[ii]])) else
      outLabel <- append(outLabel, sprintf("%s:Z", lop$grpLab[[ii]]))
   }
   if (lop$nGrp==2) {
      outLabel <- append(outLabel, sprintf("%s-%s:b", lop$grpLab[[2]],lop$grpLab[[1]]))
      if(lop$KHtest) outLabel <- append(outLabel, sprintf("%s-%s:t", lop$grpLab[[2]],lop$grpLab[[1]])) else
      outLabel <- append(outLabel, sprintf("%s-%s:Z", lop$grpLab[[2]],lop$grpLab[[1]]))
   } # if (lop$nGrp==2)
   
   if(anyCov) for(ii in 1:lop$nCov) {
      outLabel <- append(outLabel, sprintf("%s:b", lop$covName[ii]))
      if(lop$KHtest) outLabel <- append(outLabel, sprintf("%s:t", lop$covName[ii])) else
      outLabel <- append(outLabel, sprintf("%s:Z", lop$covName[ii]))
   } 
   
   if(lop$anaType==4) {for(ii in 1:lop$nGrp) {
   ...
   if(lop$resZout==1) for(ii in 1:lop$nGrp) for(jj in 1:lop$nSubj[ii]) {
      if(ii==1 & jj==1) {
         ...
         iccLabel  <- append(iccLabel, sprintf("%s-lambda", lop$subjLab[[ii]][[jj]]))
         resZLabel <- append(resZLabel, sprintf("%s-Res:Z", lop$subjLab[[ii]][[jj]]))
      }  
   }   

   ############ NEED TO MODIFY HERE!!!!!!!!!!!!!!!!!!############################
   # t and chi-sq for QE with df = sum(lop$nFiles)/2 - lop$nGrp (need changes later with more complicated models
   
   nDF <- sum(lop$nFiles)/2-lop$nGrp-lop$nCov
   
   statpar <- "3drefit"
   if (lop$KHtest) for (ii in 1:(2*lop$nGrp-1)) statpar <- paste(statpar, " -substatpar ", 2*(ii-1)+1, " fitt ", nDF) else
      for (ii in 1:(2*lop$nGrp-1)) statpar <- paste(statpar, " -substatpar ", 2*(ii-1)+1, " fizt")
   
   if(anyCov) if (lop$KHtest) for(ii in 1:lop$nCov) statpar <- paste(statpar, " -substatpar ", nBrick0-3-2*(lop$nCov-ii), " fitt ", nDF) else
      for(ii in 1:lop$nCov) statpar <- paste(statpar, " -substatpar ", nBrick0-3-2*(lop$nCov-ii), " fizt")
   
   if(lop$anaType==4) {
      statpar <- paste(statpar, " -substatpar ", nBrick0-5, " fict ", lop$nSubj[1]-lop$nCov) # Chi-sq for QE: group 1
      statpar <- paste(statpar, " -substatpar ", nBrick0-3, " fict ", lop$nSubj[2]-lop$nCov) # Chi-sq for QE: group 2
   ...
   statpar <- paste(statpar, " -view tlrc -addFDR -newid ", lop$outFN)  
                  # assume tlrc space: wrong for monkey study, for example
   cat ('statpar', statpar, '\n')
   system(statpar)
   ...
   if(lop$resZout==1) {
      ...
      write.AFNI(lop$resZ_FN, outArr[,,,seq((nBrick0+2), nBrick, by=2)], 
                 ...
                 delta=lop$myDelta, idcode="whatever")
      statparICC  <- paste(statparICC, " -view tlrc -newid ", lop$icc_FN)
      statparResZ <- paste(statparResZ, " -view tlrc -addFDR -newid ", 
                           lop$resZ_FN)
      system(statparICC)
      system(statparResZ)
   }
