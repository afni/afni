...
print("#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
print("Version 0.0.1,  May 20, 2009")
...
nGrp <- as.integer(readline("Number of groups (1 or 2)? "))
   ...
   if(anaType==3) {
      ...
      for(ii in 1:nLevel) {
         ...
         for(jj in 1:nSubj[1]) {
            bFN[[ii]][[jj]] <- readline(sprintf("No. %i subject file for beta or linear combination of betas condition %s: ", jj, conLab[[ii]]))
            tFN[[ii]][[jj]] <- readline(sprintf("No. %i subject file for the corresponding t-statistic in condition %s: ", jj, conLab[[ii]]))
            print("-----------------")
         }
         bList[[ii]] <- lapply(bFN[[ii]], read.AFNI); tList[[ii]] <- lapply(tFN[[ii]], read.AFNI)
         if(ii==1) {myNote=bList[[1]][[1]]$header$HISTORY_NOTE; myOrig=bList[[1]][[1]]$origin; myDelta=bList[[1]][[1]]$delta; myDim <- bList[[1]][[1]]$dim}
